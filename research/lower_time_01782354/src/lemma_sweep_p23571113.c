/* lemma_sweep_L_tbox.c — t-BOX variant of the verified lemma_sweep_L.c:
   ...
   Euler modes: 1 = no mollifier, 2 = P2, 3 = P23, 4 = P235,
                5 = P2357, 6 = P235711, 7 = P23571113.
   Mode 7 is represented recurrence-wise, not by a dense 64-divisor scan:
     B_13(n;N) = B_11(n;N) - b_t(13) 1_{13|n} B_11(n/13;N),
   and identically for A_13.  Patch construction scans the old P11 support
   and the shifted-by-13 support separately; activation updates process the
   32 old and 32 shifted divisor events.  Thus the maximum support is still
   30030*N, while the scan cost is about two P11 supports rather than a
   dense pass through all integers up to 30030*N.
...
/* P11-base b, a0 for given n under cutoff "n <= d*Ncut"; returns the
   smallest active base divisor (0 if none).  For modes 1--6 this is the
   complete computation. */
static ulong bA0_base(arb_t b, arb_t a0, ulong n, ulong Ncut,
                      const mollset_t *ms, const arb_t t, const arb_t y,
                      slong prec){
...
/* Complete b, a0 computation.  The P13 mode uses the exact Euler-factor
   recurrence rather than materializing the 64 divisors.  The shifted
   contribution has active divisor 13*d, so the returned dmin remains the
   smallest active divisor of the full mollifier; this is essential for the
   frozen activation gamma in Wcompute. */
static ulong bA0(arb_t b, arb_t a0, ulong n, ulong Ncut,
                 const mollset_t *ms, const arb_t t, const arb_t y,
                 slong prec){
    ulong dmin = bA0_base(b, a0, n, Ncut, ms, t, y, prec);
    if(ms->p13_recurrence && n % 13 == 0){
        arb_t bshift, a0shift;
        arb_init(bshift); arb_init(a0shift);
        ulong dshift = bA0_base(bshift, a0shift, n/13, Ncut,
                               ms, t, y, prec);
        if(dshift != 0){
            arb_submul(b, ms->b13, bshift, prec);
            arb_submul(a0, ms->b13, a0shift, prec);
            dshift *= 13;
            if(dmin == 0 || dshift < dmin) dmin = dshift;
        }
        arb_clear(bshift); arb_clear(a0shift);
    }
    return dmin;
}

/* Whether the P11-base support contains n at cutoff Ncut. */
static int base_support_active(ulong n, ulong Ncut, const mollset_t *ms){
    if(n <= Ncut) return 1;
    for(int i=1; i<ms->nd; i++){
        ulong d = ms->dvec[i];
        if(n % d == 0 && n <= d*Ncut) return 1;
    }
    return 0;
}

/* Whether the full support contains n.  In P13 mode it is the union of the
   P11 support and its 13-fold image. */
static int support_active(ulong n, ulong Ncut, const mollset_t *ms){
    if(base_support_active(n, Ncut, ms)) return 1;
    return ms->p13_recurrence && n % 13 == 0
        && base_support_active(n/13, Ncut, ms);
}
...
/* Rebuild the W moments by scanning the exact support union.  The first
   range contains every base-supported integer and any overlapping shifted
   term.  The second range enumerates only n=13*m above the base support;
   the strict partition prevents omissions and double counting. */
static void rebuild_W_moments(arb_ptr MW, slong K, ulong N, ulong Nstart,
                              const mollset_t *ms,
                              const arb_t t, const arb_t y, const arb_t pi,
                              const arb_t c002y, const arb_t ge,
                              const arb_t ce, const arb_t a0e,
                              const arb_t pc, slong prec){
    const ulong Dbase = ms->dvec[ms->nd-1];
    const ulong base_limit = Dbase*N;
    arb_t W; arb_init(W);
    for(ulong n=2; n<=base_limit; n++){
        if(!support_active(n, N, ms)) continue;
        Wcompute(W, n, N, Nstart, ms, t, y, pi, c002y, ge, ce, prec);
        add_moments(MW, K, n, W, a0e, pc, prec);
    }
    if(ms->p13_recurrence){
        const ulong first_m = base_limit/13 + 1;
        for(ulong m=first_m; m<=base_limit; m++){
            if(!base_support_active(m, N, ms)) continue;
            const ulong n = 13*m;
            Wcompute(W, n, N, Nstart, ms, t, y, pi, c002y, ge, ce, prec);
            add_moments(MW, K, n, W, a0e, pc, prec);
        }
...
int main(int argc, char *argv[]){
    ...
    if(mtype < 1 || mtype > 7){
        printf("mtype 1/2/3/4/5/6/7 only\n");
        return 2;
    }
    ...
    mollset_t ms; ms.nd = 1; ms.dvec[0] = 1; ms.p13_recurrence = 0;
    arb_init(ms.b13); arb_init(ms.ln13);
    arb_zero(ms.b13); arb_zero(ms.ln13);
    ...
    arb_set_ui(ms.moll[0],1); arb_zero(ms.lnd[0]);
    {
        ...
        if(mtype==2){ ms.nd=2; ms.dvec[1]=2; arb_neg(ms.moll[1],b2); arb_set(ms.lnd[1],ln2); }
            ...
            if(mtype==7){
                ms.p13_recurrence=1;
                arb_set_ui(tmp,13); arb_log(ms.ln13,tmp,prec);
                bt_eval(ms.b13,ms.ln13,t,prec);
            }
        }
    ...
    const ulong Dbase = ms.dvec[ms.nd-1];
    const ulong D = ms.p13_recurrence ? 13*Dbase : Dbase;
    ...
    for(ulong N = Nstart; N <= Nend; N += stride){
        ...
        if(ms.p13_recurrence){
            /* Sum |lambda_d| d^-sigma factorizes because b_t(13)>0:
               M_13(sigma) = M_11(sigma)(1+b_t(13)13^-sigma).  Taking
               abs explicitly keeps the enclosure valid even for a wide
               future t-box whose sign is not recognized by Arb. */
            arb_mul(tmp, ms.ln13, sigma, prec); arb_neg(tmp, tmp);
            arb_exp(tmp, tmp, prec);
            arb_abs(tmp2, ms.b13); arb_mul(tmp, tmp, tmp2, prec);
            arb_add_ui(tmp, tmp, 1, prec); arb_mul(modmoll, modmoll, tmp, prec);
        }
        ...
        if(mode == 'n'){
            ...
            if(ms.p13_recurrence){
                /* Shifted-only support n=13*m above Dbase*N. */
                for(ulong m=base_limit/13+1; m<=base_limit; m++){
                    if(!base_support_active(m, N, &ms)) continue;
                    const ulong n = 13*m;
                    bA0(b_, a0_, n, N, &ms, t, y, prec);
                    arb_mul(tmp, modgamma, a0_, prec);
...
#else
                    arb_sub(m1, b_, tmp, prec); arb_abs(m1, m1);
                    arb_add(m2, b_, tmp, prec); arb_abs(m2, m2);
                    arb_mul(m2, m2, cnow, prec); arb_max(m1, m1, m2, prec);
#endif
                    arb_set_ui(lnn, n); arb_log(lnn, lnn, prec);
                    arb_mul(wgt, lnn, sigma, prec); arb_neg(wgt, wgt);
                    arb_exp(wgt, wgt, prec); arb_addmul(Ssum, m1, wgt, prec);
                }
            }
            ...
            if(!have_patch){
                ...
                rebuild_W_moments(MW, K, N, Nstart, &ms, t, y, pi,
                                  c002y, ge, ce, a0e, pc, prec);
                for(ulong n=2; n<=N; n++){
                    arb_set_ui(lnn, n); arb_log(lnn, lnn, prec);
                ...
                for(ulong Np = Nactive+1; Np <= N; Np++){
                    ...
                    if(ms.p13_recurrence){
                        /* The 32 shifted divisors 13*d activate at the
                           distinct points n=13*d*Np.  Recomputing the full
                           W on both sides also captures any already-active
                           base contribution at the same n. */
                        for(int i=0;i<ms.nd;i++){
                            const ulong dd = 13*ms.dvec[i];
                            const ulong n = dd*Np;
                            Wcompute(Wold, n, Np-1, Nstart, &ms, t, y,
                                     pi, c002y, ge, ce, prec);
                            Wcompute(W, n, Np, Nstart, &ms, t, y,
                                     pi, c002y, ge, ce, prec);
                            arb_sub(W, W, Wold, prec);
                            add_moments(MW, K, n, W, a0e, pc, prec);
                        }
