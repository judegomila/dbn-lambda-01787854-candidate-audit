/*
   ...
   Derived from barrier/src/StoredSumTaylorTail_cert.c with the following
   changes and no others:

     1. The pinned anchor X=6000000185827 (slab [X, X+1]) and window
        N=690988 become command-line arguments:
        storedsum_taylor_tail_site X N.
     2. A research-status line is always printed, and the terminal line
        is prefixed with SITE so a transcript cannot be mistaken for the
        sealed checker's output.

   Every constant, threshold, subdivision, and inequality is otherwise
   verbatim.  Independent certificate for the 62 x 62 stored-sum
   truncation used at the supplied X: it sums a rigorous termwise
   exponential-tail majorant over n=1,...,N (no numerical quadrature).
*/
...
main(int argc, char *argv[])
{
    ...
    if (argc != 3) {
        fprintf(stderr, "usage: storedsum_taylor_tail_site X N\n");
        return 2;
    }

    arb_t X, Xhi, x, y, logn0, n0, t0, pi, tmp, tmp2, re, im, dabsre,
          dabsim, wr, wi, wmod, afaclog, afac, coeff, sqrtn0,
          prefactors, L, A, B, fac, IA, IB, expA, expB, term, total,
          nn, half, one, ratio;
    acb_t s, u, al, D, als, alu, q;

    arb_init(X); arb_init(Xhi);
    arb_init(x); arb_init(y); arb_init(logn0); arb_init(n0);
    ...
    arb_union(x, X, Xhi, prec);
    arb_set_str(tmp, "0.1809", prec);
    arb_union(y, tmp, one, prec);
    ...
    printf("RESULT: SITE STORED-SUM TAYLOR-TAIL CERTIFIED\n");
