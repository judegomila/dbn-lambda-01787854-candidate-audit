/*
   RESEARCH-LANE DERIVATIVE — UNSEALED.

   Derived from barrier/src/verify_uniform_error_01787854.c with the
   following changes and no others:

     1. The pinned slab anchor X=6000000185827 and window index N=690988
        become command-line arguments:  uniform_error_site X N.
     2. The two RESULT strings are prefixed with SITE so a transcript
        cannot be mistaken for the sealed checker's output, and a
        research-status line is always printed.

   Everything else — the t range [0, 129/800], ymin=0.1809, every
   constant, threshold, and inequality — is verbatim from the sealed
   program.  Certifies the uniform numerical upper bound for formulas
   (20)--(23) and the conservative 10.50 corollary of Proposition
   6.6(vi) in Polymath 15, arXiv:1904.12438v2, on

       x in [X, X+1],  y in [0.1809, 1],  t in [0, 0.16125].

   This program certifies the evaluation/majorization of those bounds.  It
   does not by itself extend the stated theorem domain
...
main(int argc, char *argv[])
{
    ...
    if (argc != 3)
    {
        fprintf(stderr, "usage: uniform_error_site X N\n");
        return 2;
    }
    ...
    if (arb_set_str(X, argv[1], prec) || !arb_is_int(X)
        || !arb_is_positive(X))
    {
        fprintf(stderr, "error: X must be a positive integer\n");
        return 2;
    }
    site_N = strtol(argv[2], NULL, 10);
    if (site_N <= 0)
    {
    ...
    printf("STATUS: UNSEALED RESEARCH ONLY; no improved Lambda bound certified.\n");
    printf("SITE: X="); arb_printn(X, 20, 0);
    printf(", N=%ld\n", site_N);
    ...
    if (!compute_exact_N(&nhi, Xhi, T, prec))
        nhi = -1;
    printf("[%s] N is %ld at both monotone slab corners (%ld,%ld)\n",
           (nlo == site_N && nhi == site_N) ? "PASS" : "FAIL",
           site_N, nlo, nhi);
    ok &= (nlo == site_N && nhi == site_N);
    ...
    */
    ...
    printf("%s\n", ok
        ? "RESULT: SITE UNIFORM NUMERICAL ERROR-FORMULA BOUND CERTIFIED"
        : "RESULT: SITE FAIL");
