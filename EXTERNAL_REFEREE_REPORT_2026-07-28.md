...
## Related: why this candidate is optimal for the verified data (operator-frame)

A separate twin-ladder analysis (private mathexplorer project, page `/frozen-rung-pt-equivalence`, module `frozen-rung-pt-equivalence-2026-07-31.js`) records a proposition relevant to how far this bound can go at fixed height: **the Platt–Trudgian verified-height Λ bound is the winding-form "frozen-rung" index certificate of the shifted-bridge kernel `K_a` (`X(w)=ξ(½+w)`), and by inertia-preservation (Sylvester congruence on `ind₋`, plus the Krein–Langer identity `ind₋(K_{R_a}) = #{ξ-zeros with Re λ > a}`) no freeze of the ladder can certify a smaller off-critical-zero count than the verified data supports** — i.e. it can never beat the verified height. The "can-never-beat" (inertia) half is a theorem; the full equivalence is conjectural (two named lemmas, one open). This is the operator-side echo of a companion floor-map (Fable, 2026-07-31) finding that `Λ ≤ 0.1787854` sits ≈2×10⁻⁷ from the certifiability wall at height 3×10¹² and that the only Λ-moving lever is more verified height (inverse-log). Both are *context* for a referee — **not** part of this candidate's proof chain.
