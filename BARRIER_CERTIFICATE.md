...
`barrier/src/verify_uniform_error_01787854.c` independently evaluates the
displayed bounds (20)--(24) with Arb on the complete box
...
paths use \(x-6.66\) in (23) and the paper's \(x-12\) in (24).
...
4. The displayed error majorant is continuous there: all denominators are
...
- independently recomputed minimum prism margin
  \[
