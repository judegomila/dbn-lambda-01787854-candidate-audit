...
exact error definitions (71)--(72) and the component estimates in
Proposition 6.6, of D. H. J. Polymath, *Effective approximation of heat flow
evolution of the Riemann \(\xi\) function, and a new upper bound for the
de Bruijn--Newman constant*, arXiv:1904.12438v2.  In particular, the target
uses the conservative \(10.50=3.58+6.92\) consequence of Proposition
6.6(vi), not the displayed \(10.44\) in equation (24); the exact implication
and every numerical consumer are audited in `ERROR_CONSTANT_WELD.md`.
...
For \(e_{C,0}\), we use the conservative corollary of Polymath
Proposition 6.6(vi) proved in `ERROR_CONSTANT_WELD.md`, with denominator
\(x-12\) and constant \(3.58+6.92=10.50\).  We deliberately do not rely on
the displayed \(10.44\) in formula (24):
...
\[
Q(L)=3\sqrt{L^2+\pi^2/4}+10.50.
\]

Since \(dQ/dx<3/x\),

\[
Q'(x)(x-12)-Q(x)<3-10.50<0.
\]
