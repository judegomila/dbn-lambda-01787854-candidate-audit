...
\hypersetup{
  pdftitle={A computer-assisted proof of the bound Lambda <= 0.1787854},
  pdfauthor={Jude Gomila},
  pdfsubject={An explicit upper bound for the de Bruijn--Newman constant},
  pdfkeywords={de Bruijn--Newman constant, Riemann xi function, interval arithmetic, computer-assisted proof}
}
...
\newcommand{\projectlink}[1]{\href{https://github.com/judegomila/dbn-lambda-01787854-candidate-audit/tree/6222740efae58d773a70ff929cf12a34932d2af0}{#1}}
\newcommand{\programlink}[2]{\href{https://github.com/judegomila/dbn-lambda-01787854-candidate-audit/blob/6222740efae58d773a70ff929cf12a34932d2af0/#1}{#2}}
...
\title{A computer-assisted proof of the bound
$\Lambda\le 0.1787854$ for the de Bruijn--Newman constant}
\author{Jude Gomila \textnormal{---}
{\small\textnormal{\emph{Special thanks to Dan Romik and Max Atkin}}}}
\date{20 August 2026}
...
\begin{abstract}
\noindent We prove that the de Bruijn--Newman constant satisfies
\[
\LB\le\frac{893927}{5000000}=0.1787854
\]
by applying the Polymath~15 barrier criterion at the exact parameters
$X=6000000185827$, $t_0=129/800$, and
$y_0^2=87677/2500000$. We show that the assumptions needed to apply the
criterion with these parameters hold by combining results of Platt and
Trudgian with new computer-assisted calculations.
\end{abstract}
...
\small
\raggedright
\noindent\textbf{Human--AI collaboration disclosure.}
This paper was developed through an iterative collaboration between Jude
Gomila and generative AI systems, with repeated cycles of mathematical
exploration, program development, testing, criticism, and revision. Jude
Gomila directed the work and accepts responsibility for the arguments,
computations, programs, and final text. Further details appear at the end of
the Introduction.
\end{framed}
\end{minipage}
\end{center}
...
\end{equation}
The function $H_0$ is a normalization of the Riemann $\xi$-function. With
\[
\xi(s)=\frac{s(s-1)}{2}\pi^{-s/2}\Gamma(s/2)\zeta(s)
\]
analytic continuation of the Fourier identity gives, for every $z\in\C$,
$H_0(z)=\frac18\,\xi\!\bigl(\frac12+\frac{iz}2\bigr)$, so that the
Riemann hypothesis (RH) is the statement that all zeros of $H_0$ are
real. De Bruijn \cite{deBruijn} proved that $H_t$ has only real zeros for
$t\ge\frac12$. Newman \cite{Newman} then proved that there is a finite
constant $\LB\le\frac12$, the \emph{de Bruijn--Newman constant}, such that
$H_t$ has only real zeros if and only if $t\ge\LB$. RH is equivalent to
$\LB\le0$.

Newman conjectured $\LB\ge0$, which was proved by Rodgers and Tao
\cite{RodgersTao}. On the other side, de Bruijn's classical upper
bound $\LB\le1/2$, its strict improvement by Ki, Kim and Lee
\cite{KKL}, and subsequent numerical work led to the bound
...
verified height $H>2.51\cdot10^{12}$), yields the bound
\begin{equation}\label{eq:ptbound}
\LB\le0.2 ,
\end{equation}
which is the relevant peer-reviewed benchmark for the present
argument. Platt and Trudgian further note
...
their theorem.  The value \eqref{eq:target} lies below both
$0.2$ and $0.19$ while consuming only the established
$3\cdot10^{12}$ verification height. A recent Zenodo preprint of
Mosaic Intelligence \cite{Mosaic} claims the intermediate value
$\LB\le0.1875$; it has not been peer reviewed. The value examined here is
numerically smaller than both $0.2$ and $0.1875$.
...
The proof instantiates the barrier criterion of
...
\subsection{Outline of the proof}\label{sec:status}

The argument has four parts.
\begin{enumerate}
\item Section~\ref{sec:criterion} states the Polymath~15 criterion and the
effective approximation to which we apply it.
\item Section~\ref{sec:hypi} obtains the required verified height from the
theorem of Platt and Trudgian.
\item Sections~\ref{sec:finite} and~\ref{sec:tail} prove the required
nonvanishing of $H_{t_0}$, first on a finite range and then on the infinite
tail.
\item Section~\ref{sec:barrier} proves the boundary nonvanishing needed for
the barrier argument, and Section~\ref{sec:assembly} completes the proof.
\end{enumerate}

The finite parts of the proof use interval arithmetic.  At each such point we
give the exact domain, the mathematical quantity evaluated, the strict
inequality returned by the program, and a link to the source.  The programs
use exact integer or rational inputs and outward-rounded FLINT/Arb balls
\cite{JohanssonArb}; Appendix~\ref{sec:programs} collects the links and explains
how the computations cover the required domains.  The source code is
available in the \projectlink{project repository}.

This preprint has not undergone independent peer review.

\paragraph{Use of generative AI.}
Generative AI systems, including Anthropic Claude and OpenAI ChatGPT/Codex,
were used in repeated, human-directed cycles of mathematical exploration,
program development, testing, criticism, and manuscript revision. Jude Gomila
selected the arguments and computations included here and accepts
responsibility for the mathematical claims, the programs, and the final text.



% ===============================================================
\section{The Polymath criterion and effective approximation}\label{sec:criterion}
% ===============================================================
...
\begin{theorem}[{Polymath criterion, specialized from
\cite[Theorem~1.2]{Polymath}}]\label{thm:criterion}
Let $X>0$, $0<t_0<\tfrac12$ and
$0<y_0<\sqrt{1-2t_0}$. Suppose that the following three conditions hold:
\begin{enumerate}
...
Section~\ref{sec:hypi} verifies condition (i) using the theorem of Platt and
Trudgian. Sections~\ref{sec:finite}--\ref{sec:tail} prove condition (ii), and
Section~\ref{sec:barrier} proves condition (iii), at the parameters
\eqref{eq:params}.

For condition (i) one uses the normalization connecting $H_0$ to
...
is the region excluded in (i). (The functional equation and the
reality symmetry $\overline{\xi(\bar s)}=\xi(s)$ are the classical
inputs; the affine algebra of this ``sign map'' is elementary.)
...
\end{equation}
where the normalizing factor is as follows.  Let $\operatorname{Log}$ be the
standard branch of the logarithm, with branch cut $(-\infty,0]$ and
imaginary part in $(-\pi,\pi]$, and on
$\C\setminus(-\infty,1]$ define
\begin{align}
M_0(s)&=\frac{\sqrt{2\pi}}{16}\,s(s-1)\pi^{-s/2}
 \exp\!\left(\left(\frac{s}{2}-\frac12\right)\operatorname{Log}\frac{s}{2}
 -\frac{s}{2}\right),\label{eq:M0}\\
\alpha(s)&=(\log M_0)'(s)
...
\label{eq:MtBt}
\end{align}
These are equations (6), (8)--(11) of \cite{Polymath}.  In particular
$M_0$, and hence $M_t$, takes values in $\C\setminus\{0\}$ on its
domain; since $(1+y-ix)/2$ is non-real whenever $x\neq0$, it avoids
the branch cut. Therefore $B_t(x+iy)\neq0$ for all $t\ge0$, $x>0$ and
...
of the barrier argument (Section~\ref{sec:barrier}).
...
\item the exact quantities (equations (16)--(18) of \cite{Polymath})
\begin{align}
\gamma&=\frac{M_t\bigl(\frac{1-y+ix}{2}\bigr)}
 {M_t\bigl(\frac{1+y-ix}{2}\bigr)},\label{eq:gammaexact}\\
s_*&=\frac{1+y-ix}{2}
 +\frac t2\alpha\!\left(\frac{1+y-ix}{2}\right),
 \label{eq:starexact}\\
\kappa&=\frac t2\left[
 \alpha\!\left(\frac{1-y+ix}{2}\right)
 -\alpha\!\left(\frac{1+y+ix}{2}\right)\right],
 \label{eq:kappaexact}
\end{align}
which satisfy, by equations (20)--(22) of \cite{Polymath}, the bounds
\begin{align}
...
Our proof deliberately does \emph{not} use the constant $10.44$
...
$1+u\le e^u$ to the second factor and replacing the denominator
$x-8.52$ by the smaller denominator $x-12$ yields the
\emph{conservative corollary}
...
$10.44$. Accordingly, our proof does not rely on (24): all error
...
% ===============================================================
\section{The verified-height condition}\label{sec:hypi}
% ===============================================================
...
\]
lies on the critical line; their computation accounts for the lowest
$12\,363\,153\,437\,138$ nontrivial zeros. Counting sign changes of the
completed zeta function on the critical line and using Turing's method to
confirm completeness yields the off-line exclusion form used here.
Condition (i) of
Theorem~\ref{thm:criterion} requires the absence of zeros with
$\frac{1+y_0}{2}\le\sigma\le1$ and $0\le T\le X/2$. Since
\[
...
$\zeta$ has a pole. This verifies condition (i).

We use the published Platt--Trudgian theorem as an input.

% ===============================================================
\section{The final-time right region: finite range}\label{sec:finite}
% ===============================================================

Condition (ii) requires nonvanishing of $H_{t_0}$ on
\begin{equation}\label{eq:hypiiregion}
x\ge x_*:=X+\sqrt{1-y_0^2},
\qquad
y_0\le y\le\sqrt{1-2t_0}.
\end{equation}
The proof splits this region at the abscissa corresponding to
the window index $N_*=3840000$: the finite range
$690988\le N\le3840000$ is handled in this section by
$3{,}149{,}013$ individual interval computations, and
...
The computations of Section~\ref{sec:stored} evaluate, for each
$N$, the majorants of
...
for $\Rea s_*$) used in the definitions of Section~\ref{sec:stored}.

\subsection{The mollified lower bound for
\texorpdfstring{$|f_{t_0}|$}{|f(t0)|}}\label{sec:native}
...
\begin{proof}
Write \eqref{eq:ft} as $f_t=A+\gamma C_\kappa$, where
\[
A=\sum_{m\le N}b_t(m)m^{-s_*},\qquad
C_\kappa=\sum_{m\le N}m^yb_t(m)m^{-\overline{s_*}-\kappa}.
\]
Exact
...
The point of Lemma~\ref{lem:native} is that any per-window lower bound for
$L_N$ uses the same normalization as the additive error in \eqref{eq:thm13}; no
further Euler-factor conversion intervenes.

\subsection{The lower bounds for \texorpdfstring{$L_N$}{L(N)}}\label{sec:stored}
...
\begin{proposition}[Lower bounds for $L_N$]\label{prop:finitewindows}
...
Table~\ref{tab:primesets}; the smallest certified lower bound is
$T_{\min}$ and is obtained at $N=690988$.
\end{proposition}
...
\begin{tabular}{@{}llr@{}}
\toprule
$\calP(N)$ & range of $N$ & lower bound for $\min L_N$\\
\midrule
$\{2,3,5,7,11\}$ & $690988\le N\le728999$ & $0.000000791366$\\
$\{2,3,5,7\}$    & $729000\le N\le818999$ & $0.000315112459$\\
$\{2,3,5\}$      & $819000\le N\le1027999$ & $0.000305788807$\\
$\{2,3\}$        & $1028000\le N\le3840000$ & $0.000309285478$\\
\bottomrule
\end{tabular}
\caption{The auxiliary prime sets \eqref{eq:PN} and, for each of the
four ranges of $N$, the lower bound for $\min_N L_N$
established by the computation described in the proof of
Proposition~\ref{prop:finitewindows}.}
\label{tab:primesets}
\end{table}
\FloatBarrier

\begin{proof}
The proof is computer-assisted.  The
\programlink{independent/prop43/prop43_proof.c}{finite-window program}
computes, for every required integer $N$, an outward-rounded interval whose
lower endpoint is a lower bound for the quantity $L_N$ in \eqref{eq:LN}.
It has two evaluation modes.  The first evaluates every term of
\eqref{eq:LN} directly.  The second groups consecutive values of $N$, freezes
each weight at the endpoint that gives the larger majorant, expands the
remaining smooth dependence in Taylor series, and adds an explicit Lagrange
remainder.  The monotonicity of $g_N$ established in
Lemma~\ref{lem:freeze} makes the frozen weights conservative.  Each computed
lower endpoint is therefore rigorously no greater than the exact value of
$L_N$.

The calculation uses exact rational inputs and outward-rounded FLINT/Arb
arithmetic at $256$ bits.  The four runs use the prime sets and integer ranges
in \eqref{eq:PN} and together cover every one of the $3{,}149{,}013$ integers
$690988\le N\le3840000$.  Every computed lower endpoint is positive; the
smallest certified lower bound in each range is displayed in
Table~\ref{tab:primesets}; across all four ranges the smallest is
$791366\times10^{-12}$, obtained for $N=690988$.  This proves the
proposition.
\end{proof}

\subsection{Transfer to the full height range}\label{sec:dini}

Proposition~\ref{prop:finitewindows} concerns the single height $y=y_0$,
while condition (ii) requires nonvanishing for all
...
between Proposition~\ref{prop:finitewindows} and the full height range is
...
term $g_N(y)C_N(y)\ge0$ is nonincreasing. Statement (a) is the
delicate one: the crude
...
the $A$-side terms. We build it up in three lemmas and one
computation.
...
\begin{proposition}[Cell inequalities]\label{prop:dinicells}
...
\begin{proof}
The
\programlink{dan-reworking/code/prop49/prop49_proof.c}{height-transfer program}
performs the finite calculation that follows.  For each prime set it
enumerates the $3^{|\calP|}$
...
L\in
\begin{cases}
[\log2,\,\log(N_{\max}d_0)],&j=0,\\
[\max(\log2,\log(N_{\min}d_{j-1})),\,\log(N_{\max}d_j)],&j\ge1,
\end{cases}
\]
and
$y\in[y_0,\sqrt{1-2t_0}]$, is covered by closed rectangles with
outward-padded rational endpoints.  Ball arithmetic proves that the exact
logarithmic boundaries, $y_0$, and $\sqrt{1-2t_0}$ lie in this cover.  On
each rectangle the program replaces $q_N$ by the smallest feasible
value $\max\bigl(N_{\min}^2,e^{2L}/d_j^2\bigr)-\frac{t_0}{16}$
(splitting the $L$-range at the transition point with overlapping
rectangles; conservative by Lemma~\ref{lem:cells}(ii)--(iii)), and forms
outward enclosures of $C_0$, $C(y)$, and $C'(y)$ from the exact identity
$r_d(L)=e^{\frac{t_0}4(\sum_{p\mid d}\log^2p+\ell_d^2)}
e^{-\frac{t_0}2L\ell_d}$, and establishes on each leaf either
$h\le0$ or the strict inequality $G\,h_+<\frac L2|C_0|$.  A rectangle is
bisected until one of these two alternatives follows.

The calculation was repeated at $180$- and $256$-bit precision.  At both
precisions every rectangle is decided, and the largest ratio satisfies
\[
\sup\ \frac{G\,h_+}{\frac L2|C_0|}\ \le\ 0.99999860767275095
\ <\ 0.9999988\ <\ 1 ,
\]
the closing comparison being made against the fixed threshold
$0.9999988$. The worst rectangle lies in the five-prime range at
$L\approx1.042$--$1.054$, $y\approx0.1997$--$0.2022$.  Since the
closed rectangles cover every cell and the upper ratio is strictly below
$1$, the required inequality holds throughout the stated domain.
\end{proof}
...
\emph{Conclusion.} By Proposition~\ref{prop:finitewindows},
...
\begin{proposition}[Uniform error bound on the finite region]
\label{prop:finiteerr}
...
\]
(The third bound is established directly and is sharper than the
sum of the two displayed bounds, which are rounded upward for
display.)
\end{proposition}

\begin{proof}
The analytic reduction is stated first at the exact time $t_0$.  The
\programlink{verifiers/verify_prop410_arb.c}{finite-error program} evaluates
the same majorants on the slightly larger rational time interval
\[
I_t=\left[\frac{129}{800},\frac{161250001}{10^9}\right],
\]
so its result is a conservative strengthening of the calculation needed here.
All integers and rational constants are constructed exactly; square roots,
logarithms, exponentials, powers, and $\pi$ are evaluated with
outward-rounded FLINT/Arb balls.  Whenever an upper endpoint is reused in a
monotone majorant, the program first converts that endpoint to an exact dyadic
number, so no midpoint approximation enters the proof.

Set $N_0:=690988$.  Evaluation at the smaller window edge
\[
x_0:=x_{N_0}=4\pi\Bigl(N_0^2-\frac{t_0}{16}\Bigr)<x_*
\]
(the strict inequality is \eqref{eq:xstarwindow}) dominates the whole region
$x\ge x_*$, $y_0\le y\le\sqrt{1-2t_0}$, $t=t_0$.  The
$x$-dependent factors used below are bounded at $x_0$; the dependence on the
changing window index is handled separately in part~(iii).
Write
$\ell_0=\log\frac{x_0}{4\pi}
=\log\bigl(N_0^2-\tfrac{t_0}{16}\bigr)$,
\[
\delta_1=\frac{t_0}4\log\Bigl(1-\frac{t_0}{16N_0^2}\Bigr)^{-1}
+\frac{t_0}{2x_0^2},
\qquad
\sigma_1=\frac{1+y_0}2+\frac{t_0}2\log N_0-\delta_1
=\frac{1+y_0}2+\frac{t_0}4\ell_0-\frac{t_0}{2x_0^2}.
\]
More generally, for $N\ge N_0$ put
\[
\delta_N:=\frac{t_0}4\log\Bigl(1-\frac{t_0}{16N^2}\Bigr)^{-1}
 +\frac{t_0}{2x_N^2},
\qquad
\widetilde\sigma_N:=\frac{1+y_0}2+\frac{t_0}2\log N-\delta_N.
\]
Thus $\widetilde\sigma_{N_0}=\sigma_1$. Both summands of
$\delta_N$ decrease with $N$, so $\delta_N\le\delta_1$ and
$\widetilde\sigma_N$ grows with $\log N$ at rate at least $t_0/2$.
The exponent estimate used in part~(i) below also gives
$\Rea s_*\ge\widetilde\sigma_N$ throughout window $W_N$.

(i) \emph{The exponent.} In \eqref{eq:resbound} the positive part
is at most $1$: since $y_0<1$ and
$8/x^2\le 8/x_0^2<3y_0<3$,
...
$(\frac x{4\pi})^{-(1+y)/4}$ decreases in $x$ and $y$, while
$-\frac{t_0}{16}\log^2\frac x{4\pi}$ decreases in $x$ and is independent
of $y$ (here $\ell_0>2$).  The factor $3^y+3^{-y}$ increases in $y$;
...
\le\frac3x$ while the numerator exceeds $10.50>3$.
Hence \eqref{eq:ec0} evaluated at $x=x_0$ and $N=N_0$, with $y=y_0$
...
\exp\Bigl(\frac{t_0\log N_0}{2(x_0-6)}\Bigr)
\]
(the quantity $\log N/(x_N-6)$ decreases in $N$).  To control the
logarithm in the last factor, put $r=x/(4\pi n^2)$.  Since $n\le N$ and
$x\ge4\pi(N^2-t_0/16)$,
\[
r\ge1-\frac{t_0}{16N^2},\qquad r\le\frac{x}{4\pi}.
\]
Moreover,
\[
\frac{x}{4\pi}\left(1-\frac{t_0}{16N^2}\right)
\ge \frac{(N^2-t_0/16)^2}{N^2}\ge1,
\]
where the last inequality follows from $N^2-t_0/16\ge N$.  Thus, whether
$r$ is below or above $1$, one has
$|\log r|\le\log(x/(4\pi))$.  Consequently
$\log^2\frac{x}{4\pi n^2}\le\log^2\frac x{4\pi}$, and
the monotonicity of $\log^2(\frac x{4\pi})/(x-6.66)$ in $x$
(using $\ell_0>2$) bounds the last factor of (23) by
$\exp\bigl(\frac{(t_0^2/16)\ell_0^2+0.626}{x_0-6.66}\bigr)-1$. For
the middle factor, $\sum_{n=1}^{N}b_{t_0}(n)\,n^{-\Rea s_*}\le
1+\sum_{n=2}^{m_0}b_{t_0}(n)\,n^{-\sigma_1}+T_N$ with $m_0=2000$,
where
$T_N:=\sum_{m_0<n\le N}b_{t_0}(n)n^{-\widetilde\sigma_N}$.
The summand
$\exp(\frac{t_0}4\log^2n-\widetilde\sigma_N\log n)$ decreases in $n$ on
$n\le N$ because $\frac{1+y_0}2-\delta_1>0$, so
$T_N\le\int_{\log m_0}^{\log N}e^{\psi(L)}\,dL$ with
$\psi(L)=(1-\widetilde\sigma_N)L+\frac{t_0}4L^2$; since $\psi$ is convex,
$T_N\le(\log N-\log m_0)\,
\max\bigl(e^{\psi(\log m_0)},e^{\psi(\log N)}\bigr)$. Finally, the
two rate inequalities
\[
...
$N\ge N_0$ (using the growth of $\widetilde\sigma_N$ just noted
and $\delta_N\le\delta_1$), so their values at $N=N_0$
dominate every window; the program proves both rate inequalities. The
product of the three bounds is its quantity \texttt{error\_ab}.

The program implements the five majorants just described, proves all twelve
domain and sign inequalities used in the monotonicity argument, and bounds the
head sum for $2\le n\le2000$ term by term over the whole time interval $I_t$.
It then compares the resulting balls with exact rational
right-hand sides.  Put $\mathcal E=e_A+e_B+e_{C,0}$.  Runs at $256$ and
$512$ bits give
\[
\begin{aligned}
e_A+e_B&<2.05702368866622\times10^{-12},\\
e_{C,0}&<\frac{233492848188649183}{10^{24}},\\
\mathcal E&<\frac{233494905212337849}{10^{24}}=E_{\max},\\
T_{\min}-\mathcal E&>5.5787109478766215\times10^{-7}.
\end{aligned}
\]
Both runs also prove directly
$\mathcal E<234\times10^{-9}$ and
$T_{\min}-\mathcal E>557\times10^{-9}>0$.  Thus both precisions prove the
same strict rational inequalities, and hence all three bounds in the
statement.
\end{proof}
...
Propositions~\ref{prop:finitewindows} and~\ref{prop:transfer}, gives
\[
...
% ===============================================================
\section{The final-time right region: infinite tail}\label{sec:tail}
% ===============================================================
...
needed for condition (ii); the last window $W_{3840000}$ of the
...
\]
Both functions decrease in $N$ for fixed $t$.  Define the frozen
constants exactly by the compact-interval maxima
\[
\widehat\delta:=\max_{t\in I_t}\delta(N_*,t),
\qquad
\widehat k:=\max_{t\in I_t}k(N_*,t).
\]
They also increase in $t$ on $I_t$, so these maxima equal their values at
$t=161250001/10^9$.
The frozen exponents are then
\[
...
\subsection{Exact convolution and the contraction
\texorpdfstring{$D$}{D}}
...
\qquad
M_{\max}=M_{\max}(N,t,y):=\sum_{d\in\calS}|\lambda_d|\,d^{-\sigma_1},
\]
\[
...
\]
(included for compatibility with the program of
Proposition~\ref{prop:tailcutoff}; see the remark after the proof),
and set
...
legitimate because the required inequality $\sigma_1>\frac t2\log N$,
the first condition in \eqref{eq:capvalid}, holds. On the $A$-side,
...
but the program of Proposition~\ref{prop:tailcutoff} below includes
it in $D$, so it is included in \eqref{eq:D} in order that the
numerical bound on $D$ majorizes the same quantity the program
computes.
\end{remark}
...
inequalities, all evaluated at $N=N_*$, uniformly for $t\in I_t$ and
$y\in I_{\rm ext}$:
\begin{equation}\label{eq:prismconditions}
...
conditions \eqref{eq:prismconditions} for this pair, that is,
...
\eqref{eq:prismconditions}. Then each of the following is nonincreasing in
...
=1-\sigma(q)\ <\ 0,
\]
using $\sigma_1(q)>1$ from Lemma~\ref{lem:capdecay}. Thus both endpoint
expressions are positive and decreasing, so their maximum and the summand are
decreasing as well.
\end{proof}
...
Assume the cutoff conditions \eqref{eq:prismconditions}--\eqref{eq:ym} hold.
...
$\sigma_2$), for which \eqref{eq:prismconditions} holds by the assumptions
of the lemma;
...
conditions \eqref{eq:prismconditions} for the caps with left endpoint
...
\eqref{eq:prismconditions} hold by the assumptions of the lemma. In
$\mathrm{E}_{C}$: the
...
\subsection{The cutoff inequalities and the tail theorem}
\label{sec:tailcert}

The computational input of the tail argument is the following.

\begin{proposition}[Cutoff inequalities]
\label{prop:tailcutoff}
The cap-validity conditions \eqref{eq:capvalid} and the cutoff
conditions \eqref{eq:prismconditions}--\eqref{eq:ym} hold, uniformly for
$t\in I_t$ and $y\in[y_0,\sqrt{1-2t}]$, and
\[
\sup_{t\in I_t,\ y\in I_{\rm box}}D(N_*,t,y)<0.999721,
\qquad
\sup_{t\in I_t,\ y\in I_{\rm box}}M_{\max}(N_*,t,y)<1.608290,
\]
\[
...
Lemma~\ref{lem:errmaj}.  In addition, for every
$N\ge N_*$, $t\in I_t$, $y\in[y_0,\sqrt{1-2t}]$, and
$x\in[X_N(t),X_{N+1}(t))$, the inequality
\[
\frac{1-D(N,t,y)}{M_{\max}(N,t,y)}
 -(e_A(x,y,t)+e_B(x,y,t)+e_{C,0}(x,y,t))
 \ >\ 0.00017352\ >\ 0 .
\]
\end{proposition}

\begin{proof}
The \programlink{dan-reworking/code/prop510/prop510_proof.c}{tail program}
evaluates the
quantities below from the exact rational parameters and the eleven rational
mollifier coefficients.  It uses outward-rounded FLINT/Arb balls and reports
an unresolved comparison if an interval overlaps a required boundary.  The
calculation was repeated at $256$ and $512$ bits.
...
simultaneously for all $(t,y)$ in the corresponding rectangle, and its
directed upper (resp.\ lower) endpoint bounds the supremum (resp.\
infimum) over that hull. More precisely, for each precision
$p\in\{256,512\}$ let $\operatorname{Up}_p(E)$ denote the exact dyadic
upper endpoint returned by Arb's directed upper-endpoint extraction for
the $p$-bit ball evaluation of an expression $E$ over $I_t$.  That run
uses
\[
\widehat\delta_p=\operatorname{Up}_p\bigl(\delta(N_*,I_t)\bigr),
\qquad
\widehat k_p=\operatorname{Up}_p\bigl(k(N_*,I_t)\bigr),
\]
which satisfy $\widehat\delta_p\ge\widehat\delta$ and
$\widehat k_p\ge\widehat k$.  It substitutes these dyadic values for
the exact maxima and extracts point lower bounds for
$\sigma_1,\sigma_2$ over each rectangle. This is conservative throughout:
increasing either frozen defect can only lower the exponents, and every
subsequent occurrence of $\sigma_i$ is through $u^{-\sigma_i}$ or
$E_{t,\sigma_i}(u)$ with $u\ge1$, both nonincreasing in $\sigma_i$.

\emph{The remaining inequalities.} (1) The cap-validity conditions
\eqref{eq:capvalid}, in the equivalent form
$\sigma_i>\frac t2\log N_*$, on both rectangles. (2) The cutoff conditions
\eqref{eq:prismconditions} for exactly the left endpoints listed there:
$a=\lfloor M/d\rfloor$ for each $d\in\calS$ at $\sigma_1$
, $a=M$ at $\sigma_2$, and
$a=M_{\rm err}$ at both $\sigma_1$ and $\sigma_2$, each with the $t$-factor evaluated at
$\min I_t$ and the $\sigma$-factor at its lower bound over
$I_{\rm ext}$. (3) The $A$-side slope condition \eqref{eq:ym}.
(4) The conditions required by
...
$t_0+y_0^2/2=\frac{893927}{5\cdot10^6}$, and the interval inclusions
$y_0\in I_{\rm box}$,
$\max I_{\rm box}\le\sqrt{1-2t}$ for all $t\in I_t$, and
$\max I_{\rm ext}\ge\sqrt{1-2t_0}$, all checked in exact integer
...
are established as strict ball comparisons against exact rationals:
\[
...
\ >\ 0.00017352 .
\]
The enclosures obtained at $512$ bits include
$D\le0.9997209094$, $M_{\max}\le1.6082891450$,
$\mathrm{E}_{AB}+\mathrm{E}_{C}\le1.16716035\times10^{-8}$, and
margin $\ge0.0001735209$. Both precision runs prove all of the displayed
inequalities. Since the intervals contain the entire stated domain, the positive lower bound proves the
proposition.
\end{proof}

\begin{proposition}[Nonvanishing on the infinite tail]\label{thm:tail}
...
Proposition~\ref{prop:tailcutoff},
\[
|f_t(x+iy)|\ \ge\ \frac{1-D(N,t,y)}{M_{\max}(N,t,y)}
\ >\ e_A+e_B+e_{C,0},
\]
where the strict final comparison is precisely the direct interval
inequality in Proposition~\ref{prop:tailcutoff}, after applying both clauses
of Lemma~\ref{lem:reduction} and Lemma~\ref{lem:errmaj}. Here
...
% ===============================================================
\section{The intermediate-time barrier}\label{sec:barrier}
% ===============================================================
...
\]
The goal of this section is the following proposition, proved after the
boundary calculation in Section~\ref{sec:prisms}.

\begin{proposition}[Nonvanishing on the barrier rectangle]\label{thm:barrier}
$H_t(z)\ne0$ for all $z\in R$ and $0\le t\le t_0$.
\end{proposition}

Proposition~\ref{thm:barrier} proves condition (iii) of
Theorem~\ref{thm:criterion}: the curved barrier region of that
condition satisfies, horizontally,
...
\subsection{Uniform effective error on the box, including
...
\begin{proof}
The
\programlink{independent/prop62/prop62_proof.c}{barrier-error program}
encloses every quantity below with outward-rounded FLINT/Arb balls at
$256$ bits, starting from the exact inputs $X$,
...
$(X,0)$ and $(X+1,t_0)$ of the box. At both corners the outward enclosure of
$\sqrt q$ lies in $[690988,690989)$, so the floor is $690988$ throughout
the box. The same calculation proves
...
$t$-dependent parts cancel against $\frac t4\log^2n$, and
$0<a_{\mathrm{eff}}<1$);
...
&\le\sum_{n=1}^{N}\frac{b_t(n)}{n^{\Rea s_*}}
\Bigl(\frac14\log n\log\frac x{4\pi n}
+\frac\pi8\log n+\frac{2\log n}{x-6}\Bigr)\\
&\quad+|\gamma|N^{|\kappa|}\sum_{n=1}^{N}
\frac{b_t(n)\,n^y}{n^{\Rea s_*}}
\left(\begin{aligned}
&\frac14\log n\log\frac x{4\pi n}
+\frac\pi8\log n+\frac{2\log n}{x-6}\\
&\quad+\frac14\Bigl(\frac\pi2+\frac8{x-6}\Bigr)
\Bigl(\log\frac x{4\pi}+\frac8{x-6}\Bigr)
\end{aligned}\right).
\end{align*}
\end{lemma}
...
\begin{proposition}[Tail-exponent inequality]\label{prop:tailexp}
...
\begin{proof}
The \programlink{dan-reworking/code/prop65/prop65_proof.c}{tail-exponent
program} encloses both sides with outward-rounded Arb balls at $256$ bits,
starting from the exact rational parameters. The
computed enclosures are
\[
\begin{aligned}
...
\end{aligned}
\]
These intervals are disjoint, with the first lying above the second, and hence
prove the strict inequality.
\end{proof}

We shall use the following strict inequalities at the exact barrier
parameters.
...
&\le0.02-\frac12\log q(X)
+\frac{t_0\log N}{2(X-6)}+\frac{\log N}2,
\end{aligned}
\]
...
$\Lambda_z(t)$, $\Lambda_t$, all $\ge0$) of the five component
functions
\[
...
The program of Proposition~\ref{prop:prisms} evaluates $D_z(t)$
and $D_t(t)$ with $t$ an
...
integrator~\cite{JohanssonIntegration}. The integrands extend
holomorphically in $u$ away from the branch cut of the logarithm, and the
calculation uses analytic logarithm and power operations on complex balls
contained in this domain.  The first sixteen summands are evaluated
separately because Lemma~\ref{lem:headtail} gives monotonicity only for
$n\ge16$; the remaining terms are bounded by the integral exactly as in that
lemma.
\end{remark}
...
where
\begin{equation}\label{eq:prismcondition}
M_i\ >\ \frac{D_{z}h}{2}
+D_{t}\,(t_{i+1}-t_i)+0.00125\,;
\end{equation}
\item[\textup{(b)}] using the clockwise convention in the computation, every
argument increment satisfies
\[
\arg\bigl(f_{t_i}(z_j)/f_{t_i}(z_{j+1})\bigr)\in(-\pi,\pi),
\]
and their sum gives winding number~$0$ for the polygon
$\bigl(f_{t_i}(z_j)\bigr)_j$ about $0$.
\end{enumerate}
...
(this is the factor $\frac12$ in \eqref{eq:prismcondition}); each disk is
...
(Lemma~\ref{lem:tzero}). Condition \eqref{eq:prismcondition} says every
...
the origin, so its argument variation in the implemented clockwise
orientation is the principal argument of the stated endpoint ratio. The
winding number of the polygon about the origin is a well-defined integer;
by (b) it is
...
\subsection{The prism decomposition}\label{sec:prisms}

The computational input of the barrier argument is the following.

\begin{proposition}[Prism decomposition]\label{prop:prisms}
There exist times $0=\tau_0<\tau_1<\dots<\tau_{883}$ with
$\tau_{882}<t_0\le\tau_{883}$ such that for each $0\le i<883$, the
conditions (a)--(b) of Lemma~\ref{lem:prism} hold for the prism
$R\times[\tau_i,\min(\tau_{i+1},t_0)]$, with margin in
\eqref{eq:prismcondition} at least $0.5198498946$ in every prism.
\end{proposition}

\begin{proof}
The \programlink{dan-reworking/code/prop612/prop612_proof.c}{barrier program}
carries out the following three phases.  It uses outward-rounded Arb balls,
and every decisive inequality is strict.

\emph{(1) Domain conditions and truncation.} At $192$-bit precision the
calculation proves the uniform
expansion-parameter
...
\emph{(2) Coefficient calculation.} Define the $62\times62$ complex matrix
\[
a_{e,k}
...
\]
and enclose each entry by direct summation. A calculation with the two Taylor
directions shows that the polynomial evaluation in phase~(3)
reproduces, exactly up to the truncation bound of phase (1),
...
\emph{(3) Covering the time interval.} Starting at $\tau_0=0$, at each
prism start $\tau_i$ the calculation evaluates the
derivative bounds $D_z(\tau_i)$ and (for the proposed next prism, with
the time parameter interval-valued over the entire closed prism)
$D_t$, by the head-plus-integral quadratures of
Section~\ref{sec:derivbox}, rounding both up to exact integers.  It then
meshes $\partial R$ with $4\lceil D_z\rceil-4$ points (spacing at
most $h=1/(\lceil D_z\rceil-1)$), proves every mesh enclosure of
$f_{\tau_i}$ to exclude zero, every argument increment to lie
strictly in $(-\pi,\pi)$, and the per-rectangle winding enclosure
to lie strictly in $(-\frac14,\frac14)$, establishing winding number $0$.
It chooses $\tau_{i+1}$ adaptively at half the established slack of
\eqref{eq:prismcondition}, as an exact dyadic number, and proves the inequality
$\min_j|f_{\tau_i}(z_j)|>\frac{D_zh}2+D_t(\tau_{i+1}-\tau_i)
+0.00125$. Adjacent intervals have identical exact dyadic endpoints, the
first time is exactly $0$, the final endpoint satisfies
$\tau_{883}\ge\frac{129}{800}=t_0$, and the aggregate winding enclosure
lies strictly in $(-\frac14,\frac14)$. (The final prism may
extend slightly beyond $t_0$; restricting it to its intersection
with $[0,t_0]$ only shrinks the displacement terms in
\eqref{eq:prismcondition}, so the conclusion for the full final prism covers
the truncated one, as required by the statement.)

The calculation produces $883$ prisms, with aggregate winding enclosure
$[\pm1.18\times10^{-14}]$ and worst margin
$0.51984989461387254365\pm2\times10^{-20}$, which lies above the
exact rational threshold $5198498946\times10^{-10}$. The first time is
exactly $0$, consecutive intervals share exact dyadic endpoints, and the last
endpoint is at least $t_0$; hence the $883$ closed prisms cover the whole time
interval. This proves the proposition.
\end{proof}

\begin{proof}[Proof of Proposition~\ref{thm:barrier}]
By Proposition~\ref{prop:prisms} and Lemma~\ref{lem:prism},
$H_\tau(z)\neq0$ for $z\in R$ and
...
$0<y_0^2<1-2t_0$. Condition (i) of Theorem~\ref{thm:criterion}
holds by Section~\ref{sec:hypi}, since $X/2<T_{\rm PT}$. Condition
(ii) holds by combining Proposition~\ref{prop:finitelane}
(nonvanishing of $H_{t_0}$ for $x_*\le x<x_{3840001}$,
$y_0\le y\le\sqrt{1-2t_0}$) with Proposition~\ref{thm:tail} applied at
$t=t_0\in I_t$ (nonvanishing for $x\ge x_{3840000}=X_{N_*}(t_0)$):
the two ranges overlap on the full window $W_{3840000}$, so together
they cover all $x\ge x_*$. Condition (iii) holds by
Proposition~\ref{thm:barrier}, since the curved barrier region is
...
\appendix
\section{Programs used in the proof}\label{sec:programs}

The calculations in Propositions~\ref{prop:finitewindows}, \ref{prop:dinicells},
\ref{prop:finiteerr}, \ref{prop:tailcutoff}, \ref{prop:barriererr},
\ref{prop:tailexp}, and \ref{prop:prisms} use exact integer or rational
inputs and outward-rounded FLINT/Arb arithmetic.  The source links in
Table~\ref{tab:programs} point to Git commit
\texttt{\seqsplit{6222740efae58d773a70ff929cf12a34932d2af0}}.  Each program
returns a mathematical inequality only when the relevant intervals are
disjoint; an overlapping interval is reported as unresolved.

\begin{table}[ht]
\centering
\small
\begin{tabular}{@{}p{0.15\linewidth}p{0.22\linewidth}@{\hspace{1em}}p{0.52\linewidth}@{}}
\toprule
Result & Source & Calculation\\
\midrule
Prop.~\ref{prop:finitewindows} &
\programlink{independent/prop43/prop43_proof.c}{Program P4.3} &
Evaluates $L_N$ for every $690988\le N\le3840000$ and proves
$L_N\ge791366\times10^{-12}$.\\
Prop.~\ref{prop:dinicells} &
\programlink{dan-reworking/code/prop49/prop49_proof.c}{Program P4.9} &
Covers the four $(L,y)$ domains by closed rectangles and proves
$Gh_+<\frac L2|C_0|$ on every rectangle.\\
Prop.~\ref{prop:finiteerr} &
\programlink{verifiers/verify_prop410_arb.c}{Program P4.10} &
Computes the five finite-region error majorants at $256$ and $512$ bits and
proves $e_A+e_B+e_{C,0}<E_{\max}<234\times10^{-9}$.\\
Prop.~\ref{prop:tailcutoff} &
\programlink{dan-reworking/code/prop510/prop510_proof.c}{Program P5.10} &
Evaluates the cutoff contraction and error terms on
$I_t\times I_{\rm box}$ and
proves a post-error margin greater than $0.00017352$.\\
Prop.~\ref{prop:barriererr} &
\programlink{independent/prop62/prop62_proof.c}{Program P6.2} &
Proves that the window index is constant on the barrier box and that the
effective-approximation error is less than $0.00125$.\\
Prop.~\ref{prop:tailexp} &
\programlink{dan-reworking/code/prop65/prop65_proof.c}{Program P6.5} &
Encloses both sides of the tail-exponent inequality and proves that their
difference is positive.\\
Prop.~\ref{prop:prisms} &
\programlink{dan-reworking/code/prop612/prop612_proof.c}{Program P6.12} &
Builds the Taylor coefficients, covers $[0,t_0]$ by $883$ closed prisms, and proves
zero winding with a minimum margin greater than $0.5198498946$.\\
\bottomrule
\end{tabular}
\caption{Programs used for the finite calculations.  The displayed labels are
direct links to the corresponding source files in the
\projectlink{project repository}.}
\label{tab:programs}
\end{table}

The $256$- and $512$-bit outputs for Program P4.10 are available as
\programlink{logs/prop410_arb_256.log}{the 256-bit output} and
\programlink{logs/prop410_arb_512.log}{the 512-bit output}.
...
in \emph{Mathematical Software -- ICMS 2018}, Lecture Notes in Comput.\ Sci.\
...
\bibitem{Mosaic}
Mosaic Intelligence,
\emph{A certified unconditional upper bound $\Lambda\le0.1875$ for the
de Bruijn--Newman constant}, Zenodo preprint (3 July 2026).
\href{https://doi.org/10.5281/zenodo.21175533}{doi:10.5281/zenodo.21175533}.
...
Res.\ Math.\ Sci.\ \textbf{6} (2019), article 31.
...
Special thanks to Dan Romik and Max Atkin.  Dan Romik gave an extensive
critical reading, reorganized substantial parts of the argument, and developed
separate programs reproducing several of the computations.  I am grateful to
Max Atkin for discussions and feedback.
