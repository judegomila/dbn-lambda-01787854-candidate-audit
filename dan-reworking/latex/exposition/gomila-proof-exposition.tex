...
\begin{proof}
The proof is computer-assisted.  The authoritative verification
program is the standalone FLINT/Arb implementation
\texttt{verify\_prop410\_arb.c}, included with this manuscript as
supplementary material (directory \texttt{code/prop410}; sealed in
the audit repository as \texttt{verifiers/verify\_prop410\_arb.c})
and run independently at $256$ and $512$ bits.  It computes
rigorous directed Arb ball enclosures of majorants of the
right-hand sides of equation (23) of \cite{Polymath} and of the
corollary \eqref{eq:ec0}.  The earlier \texttt{mpmath} interval
program \texttt{prop410\_proof.py} ($220$-bit precision; derived
from Gomila's source as documented in its header and reviewed line
by line as part of this audit) computes the same majorants and is
retained alongside it as corroboration; because it shares the
\texttt{mpmath.iv} backend and a line-for-line identical budget
routine with the repository's original verifier, it is a
same-backend replay, not an independent numerical implementation.
The programs' only inputs are the exact rational parameters
$t_0$, $y_0^2$ and $N_0=690988$ (the enclosures are computed over
the rational interval $[t_0,\,t_0+10^{-9}]$, which contains
$t_0$); they read no stored data.  Every acceptance condition
below fails closed: in the Arb program each decisive inequality
subtracts the exact rational bound and requires the entire
resulting ball to be strictly on the winning side, so an
overlapping or indeterminate comparison is a failure, and every
decimal constant of the formulas is constructed from an exact
rational ($0.02=1/50$, $0.626=313/500$, $1.24=31/25$,
$6.66=333/50$, $10.50=21/2$, $0.125=1/8$).
...
Both programs certify the twelve domain and sign conditions
...
A note on re-execution, as distinct from re-reading. All eight
programs can be re-run from what is supplied.
Propositions~\ref{prop:dinicells} and~\ref{prop:tailexp} take no
arguments; Proposition~\ref{prop:tailcert} takes an optional
precision defaulting to $320$ bits; the Arb program of
Proposition~\ref{prop:finiteerr} requires an explicit precision
and refuses anything below $256$ bits; the report of
Proposition~\ref{prop:prismcert} records the three parameters that
affect its arithmetic, its fourth argument controlling only output
verbosity; and the remaining reports record the parameters used.

Proposition~\ref{prop:sweep} deserves a word, since its program
takes twelve positional arguments and its stored sweep outputs
record only the range, the mollifier type, the $t$-box and the
weight. The remaining arguments are nonetheless fully determined:
they are fixed by the schedule in the repository's own
\texttt{scripts/run\_full\_sweep.sh}, which specifies for each
mollifier band the working precision, the Taylor order and the
half-width, and which is itself part of the sealed inventory. That
schedule, rather than any particular sharding of it, is the
authoritative specification for a replay; it is transcribed in
\texttt{independent/prop43/INVOCATIONS.md}.
