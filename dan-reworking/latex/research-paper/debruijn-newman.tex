...
\title{An improved upper bound for the de Bruijn--Newman
constant\thanks{Working draft, circulated for review. The argument
has not been peer reviewed. Its seven computer-assisted
propositions have been reproduced by an independent referee, on a
separately built toolchain, by standalone programs that read no
stored data; an overall certification of correctness awaits the
completion of that review. Remove or revise this note on
submission.}}
\author{Jude Gomila \and [additional authors TBD]}
\date{}
...
\begin{abstract}
We prove the new upper bound
$\LB\le\frac{893927}{5000000}=0.1787854$
for the de Bruijn--Newman constant $\LB$, improving the bound
$\LB\le0.22$ of the Polymath~15 project and the previous record
$\LB\le0.2$ of Platt--Trudgian. The proof is unconditional: it
instantiates the criterion of Polymath~15 [Theorem~1.2,
arXiv:1904.12438v2] at the exact parameters $X=6000000185827$,
$t_0=129/800$, $y_0^2=87677/2500000$, supplying its three
hypotheses by, respectively, the Platt--Trudgian finite
verification of the Riemann hypothesis, a hybrid
finite-scan/analytic-tail nonvanishing theorem for $H_{t_0}$, and
an interval-arithmetic barrier argument. The proof is
computer-assisted: its computational ingredients are stated as
explicit propositions, each proved by a self-contained program
included with this paper as supplementary material. The programs
take only exact integer and rational inputs fixed in their source
code, read no stored data, and decide every acceptance condition
by a strict directed interval comparison.
\end{abstract}
...
\LB\le0.2 ,
\end{equation}
the best previously known. Platt and Trudgian further note
...
goes past both the previous record \eqref{eq:ptbound} and that
conditional entry, while consuming only the established
$3\cdot10^{12}$ verification height.

\subsection{Main result}

The main result of this paper is the following improvement of
\eqref{eq:ptbound}.
...
\subsection{Structure of the proof, and the computer-assisted
components}\label{sec:status}
...
\item a collection of new lemmas, proved below, which bridge the
published estimates to explicitly computable quantities; and
\item seven computer-assisted propositions, proved by rigorous
interval arithmetic (FLINT/Arb in C, plus one exact-rational Python
component), totalling $3{,}149{,}013$ finite ``window'' rows, a
cell-cover computation, two uniform error bounds, a standalone tail
computation, and an $883$-prism barrier computation.
\end{enumerate}
The statements quoted from the published inputs in item~(1) are
Theorems~1.2 and~1.3 of \cite{Polymath} together with the displays
(14), (20)--(24), (71)--(72), Proposition~6.6 and Lemma~8.4 of that
paper, and Theorem~1 of \cite{PlattTrudgian}; all equation and
theorem numbers below refer to the arXiv versions 1904.12438v2 and
2004.09765v1. The one point where our usage deliberately departs
from a cited display, namely equation~(24) of \cite{Polymath}, is
discussed in Remark~\ref{rem:1044}. Each computer-assisted
proposition of item (3) --- the main finite computation
(Proposition~\ref{prop:sweep}), the Dini cell computation
(Proposition~\ref{prop:dinicells}), the finite-region error bound
(Proposition~\ref{prop:finiteerr}), the tail cutoff computation
(Proposition~\ref{prop:tailcert}), the barrier-box error bound
(Proposition~\ref{prop:barriererr}), the tail-exponent inequality
(Proposition~\ref{prop:tailexp}), and the barrier prism
decomposition (Proposition~\ref{prop:prismcert}) --- is proved by a
standalone program included with this paper (supplementary
directory \texttt{code/}); the programs and their common discipline
are described with the respective propositions and summarized in
...
The work presented here was done with substantial assistance from Anthropic's Claude AI model.
...
the present paper); we use it as a published-theorem input.
...
The point of Lemma~\ref{lem:native} is that any
...
\subsection{The lower bounds for $L_N$}\label{sec:stored}
...
\begin{proposition}[Lower bounds for $L_N$]\label{prop:sweep}
...
the program \texttt{prop43\_proof.c}, included with
this paper as supplementary material (directory
...
Each printed value is therefore a lower bound for $L_N$.
The program has
...
rigorous Taylor moments with an explicit Lagrange remainder. Every
step of both modes is conservatively directed, and the two modes
produce the identical bound at the extremal index $N=690988$.

The program was compiled against FLINT/Arb~3.6.0 and run at 256-bit
precision over the four
...
values of $N$ produced a positive bound, with no row left
undetermined; the concatenated outputs cover
$690988\le N\le3840000$ with no gaps or duplicates; and the
per-range minima are as in Table~\ref{tab:primesets}, with
...
\begin{proof}
The proof is computer-assisted; the program
\texttt{prop49\_proof.c}, included with this paper as
supplementary material (directory \texttt{code/prop49}), reads no
stored data and fails closed at every decision.
...
$y$-box, is itself checked by ball arithmetic before use. On
...
failing closed on any unresolved leaf. Before starting, the
program also checks numerically, at the
...
\begin{proof}
The proof is computer-assisted.  The authoritative program is the
standalone FLINT/Arb implementation
\texttt{verify\_prop410\_arb.c}, included with this paper as
supplementary material (directory \texttt{code/prop410}) and run
independently at $256$ and $512$ bits; it computes rigorous
directed Arb ball enclosures of majorants of the right-hand sides
of equation (23) of \cite{Polymath} and of the corollary
\eqref{eq:ec0}.  The \texttt{mpmath} interval program
\texttt{prop410\_proof.py} ($220$-bit precision) computes the same
majorants and is retained as corroboration; it shares its
numerical backend with the audit repository's original verifier
and is therefore a same-backend replay rather than an independent
numerical implementation.  The programs' only inputs are the exact
rational parameters $t_0$, $y_0^2$ and $N_0=690988$ (the
enclosures are computed over the rational interval
$[t_0,\,t_0+10^{-9}]$, which contains $t_0$); they read no stored
data.  Every acceptance condition below fails closed: in the Arb
program each decisive inequality subtracts the exact rational
bound and requires the entire resulting ball to be strictly on the
winning side, so an overlapping or indeterminate comparison is a
failure, and every decimal constant of the formulas is constructed
from an exact rational ($0.02=1/50$, $0.626=313/500$,
$1.24=31/25$, $6.66=333/50$, $10.50=21/2$, $0.125=1/8$).
...
conditions checked by the program. Write
...
is at most $1$: the program checks $8/x_*^2<3y_0$, so
...
$-\frac{t_0}{16}\log^2\frac x{4\pi}$ decrease in $x$ (checked:
...
\le\frac3x$ while the numerator exceeds $10.50>3$ (checked).
...
(checked: $\ell>2$) bound the last factor of (23) by
...
$n\le N$ (checked: $\frac{1+y_0}2-\delta_1>0$), so
...
least $\frac{t_0}2$, checked via $N_0^2/(N_0^2-\frac{t_0}{16})>1$,
and that $\delta_N\le\delta_1$), so their values at $N=N_0$
dominate every window; both rate inequalities are checked by the
program. The product of the three bounds is the
program's quantity \texttt{error\_ab}.

Both programs check the twelve domain and sign conditions
required above and compute the enclosures of
\texttt{error\_ab}, \texttt{error\_c0} and of their sum.  The
authoritative Arb program establishes the three displayed
inequalities as strict ball inequalities against the exact
rational constants, identically at $256$ and $512$ bits; the
\texttt{mpmath} replay reproduces the same digits by exact
rational comparison of outward-rounded upper endpoints.  Each run
completes in under a second; the Arb transcripts are
\texttt{code/prop410/runs/prop410\_arb\_report\_256.txt} and
\texttt{code/prop410/runs/prop410\_arb\_report\_512.txt}, and the
replay's report is
...
Proposition~\ref{prop:tailcert}; see the remark after the proof),
and set
...
but the program of Proposition~\ref{prop:tailcert} below includes
...
\begin{proof}
The proof is computer-assisted; the program
\texttt{prop510\_proof.c}, included with this paper as
supplementary material (directory \texttt{code/prop510}),
establishes every
...
inequality, which fails closed when enclosures overlap; and it is
...
\emph{What is checked.} (1) The cap-validity conditions
...
\ >\ 0.00017352 .
\]
(The program additionally performs two-sided ``corridor'' checks on
each constituent quantity, which expose missing or duplicated
terms.) The enclosures obtained
at $512$ bits include
...
\begin{proof}
The proof is computer-assisted; the program
\texttt{prop62\_proof.c}, included with this paper as
supplementary material (directory \texttt{code/prop62}), encloses
every quantity below in
...
checks that both integers equal $690988$. It further checks
...
$t$-dependent parts cancel against $\frac t4\log^2n$; checked:
$0<a_{\mathrm{eff}}<1$);
...
\begin{proof}
The proof is computer-assisted; the program
\texttt{prop65\_proof.c}, included with this paper as
supplementary material (directory \texttt{code/prop65}),
encloses both sides in $256$-bit ball arithmetic from the exact
rational parameters and establishes the displayed strict
inequality as a strict ball comparison, which fails closed. The
...
is also re-checked at run time by the program of
...
$c_0-\frac12\log N
=\frac14\log\frac{q(X)}{N^2}-\frac{h(X,y_{\min})_+}{2X^2}$.
(The program of Proposition~\ref{prop:prismcert} also re-checks
(C2), (C4) and (C7) at run
...
The program of Proposition~\ref{prop:prismcert} evaluates $D_z(t)$
and $D_t(t)$ with $t$ an
...
ball touches the cut. Two points here require care relative to the
Polymath-era barrier code from which the program descends: the
discrete sums must not be bounded by the expression
$F(1)+\int_1^NF$ (the summand
is not monotone near $n=1$; Lemma~\ref{lem:headtail} gives
monotonicity only from $n=16$ on), whence the head-plus-integral
form of $D_z,D_t$; and the integrand callbacks must not project
the integration variable to its real part, which would void
...
\subsection{The prism decomposition}\label{sec:prismcert}

The computational input of the barrier argument is the following.

\begin{proposition}[Prism decomposition]\label{prop:prismcert}
...
\begin{proof}
The proof is computer-assisted; the program
\texttt{prop612\_proof.c}, included with this paper as
supplementary material (directory \texttt{code/prop612}; portions
of the program descend from the Polymath~15 codebase), is fully
...
establishes, at $192$-bit precision: the uniform
expansion-parameter
...
ball enclosure. A direct calculation with the two Taylor
directions shows that the
...
re-checks the conditions (C1)--(C7) of
...
to lie strictly in $(-\frac14,\frac14)$, establishing the integer
winding number $0$; chooses $\tau_{i+1}$ adaptively at half the
established slack of \eqref{eq:gate}, as an exact dyadic number; and
...
\eqref{eq:gate}, so the conclusion for the full final prism covers
the truncated one, as required by the statement.)

The run establishes $883$ prisms, with aggregate winding enclosure
$[\pm1.18\times10^{-14}]$ and worst margin
$0.51984989461387254365\pm2\times10^{-20}$, checked against the
exact rational threshold $5198498946\times10^{-10}$. The run
completes in under eight minutes; its output is
...
\section{Remarks on the computer-assisted components}
\label{sec:gaps}
% ===============================================================

The seven computer-assisted propositions of this paper ---
\ref{prop:sweep} (window sweep), \ref{prop:dinicells} (Dini
cells), \ref{prop:finiteerr} (finite-region error bound),
\ref{prop:tailcert} (tail cutoff inequalities),
\ref{prop:barriererr} (barrier-box error bound),
\ref{prop:tailexp} (tail-exponent inequality) and
\ref{prop:prismcert} (prism decomposition) --- are proved by eight
standalone programs included with this paper as supplementary
material (directory \texttt{code/}, with the outputs of the runs
in the \texttt{runs/} subdirectories). The programs share a common
discipline:
\begin{itemize}
\item all inputs are exact integers and rationals fixed in the
source code, and no stored data is read at run time: every number
entering Theorem~\ref{thm:main} is either proved analytically in
this paper or computed from the exact inputs by one of the
programs;
\item all nonexact arithmetic is rigorous interval arithmetic ---
FLINT/Arb ball arithmetic in the seven C programs, outward-rounded
\texttt{mpmath} interval arithmetic in the one Python program (the
corroborating same-backend replay of
Proposition~\ref{prop:finiteerr}, whose authoritative program is
its Arb implementation) ---
with directed endpoint extraction wherever a one-sided bound is
formed;
\item every acceptance condition is a strict directed comparison
(a strict ball inequality, or a strict inequality between exact
rationals) and fails closed: an overlapping or indeterminate
comparison aborts the run;
\item every constant displayed in the statements of the
propositions is compared explicitly against an exact rational, so
the statements assert no more than the programs establish.
\end{itemize}
The total running time of all eight programs is under one hour on
a single laptop core (the window sweep of
Proposition~\ref{prop:sweep} accounts for most of it; every other
program finishes in seconds to minutes), so full replication of
the computational part of the proof is straightforward.

The thinnest quantitative margins in the proof are the
finite-range post-error margin ($5.6\times10^{-7}$, Proposition
\ref{prop:finitelane}), the height-transfer ratio
($1-\text{ratio}\approx1.4\times10^{-6}$, Proposition
\ref{prop:dinicells}), and the tail contraction
($1-D\approx2.8\times10^{-4}$, Proposition \ref{prop:tailcert}).
The trust base is the standard one for computer-assisted proofs of
this kind: the correctness of the FLINT/Arb library, of the
compiler, and of the correspondence between the programs and the
statements of this paper, which any reader can check against the
included sources.
