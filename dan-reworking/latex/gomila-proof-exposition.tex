...
main finite computation (Proposition~\ref{prop:sweep}) and the
finite-region error bound (Proposition~\ref{prop:finiteerr}) have
been reproduced on an independent toolchain with their programs
reviewed --- their proofs are given in Sections~\ref{sec:stored}
and~\ref{sec:finiteerr} --- while the
...
(i) \emph{The exponent.} In \eqref{eq:gsk} the positive part is at
most $1$: the program certifies $8/x_*^2<3y_0$, so
...
$n\le N$, the bounds \eqref{eq:gsk} give, uniformly on the region,
\[
...
removed from this list: the computer-assisted proofs of
Propositions~\ref{prop:sweep} and~\ref{prop:finiteerr} have been
reproduced on an independent toolchain and their programs reviewed
--- see the proofs given in Sections~\ref{sec:stored}
and~\ref{sec:finiteerr} --- the proofs of
...
for Propositions~\ref{prop:sweep} and~\ref{prop:finiteerr} in
Sections~\ref{sec:stored} and~\ref{sec:finiteerr}.
