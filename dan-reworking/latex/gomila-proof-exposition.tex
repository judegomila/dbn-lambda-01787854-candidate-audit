...
independent review; it does not currently certify its correctness. All gaps and
steps that this expositor has not verified are marked explicitly.}}
\date{July 27, 2026}
...
certificate. The proof has not been peer reviewed, and the
correctness of neither the analytic reductions nor the extensive interval
computations has been independently confirmed. All statements quoted
here from the published literature have been checked against the arXiv
sources of the cited papers. The steps of the argument that
remain unverified --- at the present stage of the review, the
independent reproduction of its certified computations --- are
marked throughout as numbered \emph{gap / review points}, and are
collected in Section~\ref{sec:gaps}.
\end{abstract}
...
the course of this review. Of the computations in item (3), the
main finite computation (Proposition~\ref{prop:sweep}) has been
reproduced on an independent toolchain with its program reviewed
--- its proof is given in Section~\ref{sec:stored} --- while the
remaining certified computations have not yet been independently
verified; each such step is flagged by a framed, numbered
\emph{gap / review point}, and all remaining flagged steps are of
this computational kind. The full list is collected in
...
$10.44$. Accordingly, the current proof relies on (24): all error computations use only
...
\eqref{eq:gsk} \emph{frozen at the left edge} $x=x_N$ of the window.
...
Note that $e^{y/50}=e^{0.02y}$, matching \eqref{eq:gsk}; $G,K,\Sigma$
...
\begin{proposition}[Certified lower bounds for $L_N$;
computer-assisted]\label{prop:sweep}
...
\begin{proposition}[Certified cell inequalities;
computer-assisted]\label{prop:dinicells}
...
The proof is a computation: the program
\texttt{verifiers/verify\_triangle\_y\_dini\_arb.c} covers the
$(L,y)$-domain of each of the $3^{|\calP|}$ cells by outward-rounded
rectangles ($297490$, $52239$, $9290$ and $237$ leaves for the four
prime sets), freezes $q_N$ per Lemma~\ref{lem:cells}(iii), and on
each leaf certifies either $h\le0$ or the strict ratio bound
$G\,h_+<\frac L2|C_0|$; the worst certified ratio is
$0.99999860767275095<1$, at both 180- and 256-bit precision.

\begin{gap}{the Dini cell computation}
\label{gap:dinicells}
This gap is the computer-assisted proof of
Proposition~\ref{prop:dinicells}, certified by
\texttt{verifiers/verify\_triangle\_y\_dini\_arb.c} and not yet
independently reproduced or reviewed. Its margin
($1-\text{ratio}\approx1.4\times10^{-6}$ at the worst leaf) is the
second-thinnest in the entire proof. Review should also cover the
containment of the padded binary64 rectangle endpoints used by the
cover.
\end{gap}
...
\begin{proposition}[Uniform error bound, finite region;
computer-assisted]\label{prop:finiteerr}
...
The proof is again an interval computation (the majorants are
decreasing in $x$, so the enclosure is controlled by the left
endpoint $x_*$); unlike Proposition~\ref{prop:sweep}, it has not yet
been independently reproduced.

\begin{gap}{the finite-region error computation}
\label{gap:finiteerr}
This gap is the computer-assisted proof of
Proposition~\ref{prop:finiteerr}. In Gomila's repository the
computation is part of
\texttt{verifiers/verify\_finite\_and\_binding.py}; its verification
requires reading that program's error evaluation against equation
(23) of \cite{Polymath} and the corollary \eqref{eq:ec0}, and
rerunning it --- a small computation, in contrast with
Proposition~\ref{prop:sweep}, since the majorants are monotone in
$x$ and only the extremal enclosures enter.
\end{gap}
...
\eqref{eq:gsk} are decreasing in $x$, hence maximal at the window's
...
\]
the logarithmic term of \eqref{eq:gsk} is at least
$\frac t2\log N$ minus the first summand of $\delta(N,t)$. The
subtracted positive-part term of \eqref{eq:gsk} is at most
...
\begin{proposition}[Certified cutoff inequalities; computer-assisted]
\label{prop:tailcert}
...
The proof is a standalone FLINT/Arb computation run at
both 256 and 512 bits of precision, with a separate Python interval
implementation as cross-corroboration.

\begin{gap}{the tail computation}\label{gap:tailcomp}
This gap is the computer-assisted proof of
Proposition~\ref{prop:tailcert}: the stated bounds are machine
output of a single C program (\texttt{verify\_tail\_arb.c}). The
margin $1-D<0.000279$ is small: the contraction only barely closes.
The program's proof-to-code map (Section~10 of the Gomila's tail
note) should be audited check-by-check, and the computation
reproduced with an independently built FLINT/Arb.
\end{gap}
...
\begin{proposition}[Uniform error bound on the barrier box;
computer-assisted]\label{prop:barriererr}
...
Gomila's proof is a 256-bit Arb enclosure over the full
closed box; the constancy of $N$ makes $f_t$ a single analytic
expression on $R$, and $N^2<x/(4\pi)$ fixes the sign of the heat
exponent and yields the uniform bound $|\gamma|n^y\le e^{0.02}$ for
$1\le n\le N$. The round number $0.00125=\frac1{800}$ is the
\emph{approximation allowance} used by the prism criterion below;
\eqref{eq:barriererror} says it is safely conservative.

\begin{gap}{the uniform-error computation}
\label{gap:unifboxerr}
This gap is the computer-assisted proof of
Proposition~\ref{prop:barriererr} --- the window-index constancy
$N=690988$, the inequality $N^2<x/(4\pi)$, and the error enclosure
\eqref{eq:barriererror} --- certified by the program
\texttt{barrier/src/verify\_uniform\_error\_01787854.c}. It is a
small computation (a single 256-bit enclosure over the closed box),
not yet independently reproduced or reviewed.
\end{gap}
...
\begin{proposition}[Certified tail-exponent inequality;
computer-assisted]\label{prop:tailexp}
...
The proof is a fail-closed Arb comparison (the ``derivative-tail
exponent gate'') in the barrier program
\texttt{barrier/src/TloopSinglemat\_closed\_cert.c}. Like
$N^2<q(X)$, the inequality is tight --- the left side is of order
$10^{-7}$ --- and only a computation can separate the two sides;
its verification is part of gap~\ref{gap:barriercomp}.
...
\emph{Step 2: the $B$-side.} By \eqref{eq:gsk} and Step 1,
...
\emph{Step 3: the $A$-core.} By \eqref{eq:gsk},
...
\emph{Step 4: the $A$-side brackets.} On $[X,X+1]\times[y_{\min},1]$,
\[
...
\begin{proposition}[Certified prism decomposition;
computer-assisted]\label{prop:prismcert}
...
Gomila's proof is the sealed $883$-prism Arb computation. Its
recorded data comprise: consecutive prisms with byte-identical
adjacent time endpoints, first time exactly $0$, final endpoint
enclosing $\frac{129}{800}$; at each initial time, mesh enclosures
of $f_{\tau_i}$ on $\partial R$, the derivative bounds of
Lemma~\ref{lem:derivbox} with $D_t$ interval-evaluated on the entire
proposed prism (not sampled at its initial time), and the polygon
conditions of Lemma~\ref{lem:prism}(b), the aggregate winding
enclosure being $[-8.95,8.95]\times10^{-13}$; any overlapping or
indeterminate interval comparison counts as failure. Two full
transcripts from distinct toolchains (Linux/GCC/FLINT~3.0.1 and
macOS/Clang/FLINT~3.6.0) pass a 54-check fail-closed parser. The
$62\times62$ complex coefficient matrix used to evaluate $f_t$ is
stored to 20 decimal digits; the verifier restores each component as
a ball of radius $10^{-20}\max(1,|\text{component}|)$, independently
regenerates all $7688$ components as Arb balls (all contained, worst
radius-to-allowance ratio $<0.018$), and bounds the omitted Taylor
tail by $1.955\times10^{-22}<10^{-20}$, propagating the $10^{-20}$
radius through every value. (Restricting the final prism to its
intersection with $[0,t_0]$ only shrinks the displacement terms in
\eqref{eq:gate}, so the certificate for the full final prism covers
the truncated one.)

\begin{gap}{the barrier computation}
\label{gap:barriercomp}
This gap is the computer-assisted proofs of
Propositions~\ref{prop:prismcert} and~\ref{prop:tailexp}, both
certified by \texttt{barrier/src/TloopSinglemat\_closed\_cert.c}:
the prism certificate, the coefficient regeneration, and the
tail-exponent gate are machine output whose correctness rests on
the C sources and FLINT/Arb. The repository provides a containerized fresh-replay path
(\texttt{scripts/run\_barrier\_replay.sh}) that an independent
referee should execute on an independently built toolchain. The
provenance of the $62\times62$ coefficient matrix (that it is the
Taylor-coefficient matrix of the intended function $f_t$, not merely
a self-consistent regeneration) also requires review.
\end{gap}
...
\section{Summary of gaps and items requiring verification}
\label{sec:gaps}
% ===============================================================

For the reader's convenience we collect the numbered gap / review
points marked throughout the paper. Every remaining gap concerns
the same kind of step: the independent verification of a
computer-assisted proof component --- a proposition whose proof is
a certified computation that has not yet been reproduced or whose
program has not yet been reviewed. \emph{None}
of these is presented by the candidate as a known error; each is a
step whose validation is the task of the ongoing review, and an
adverse finding at any \emph{load-bearing} point could invalidate
Theorem~\ref{thm:main}. (The published theorem inputs themselves are
not listed here: as explained in Section~\ref{sec:status}, the
statements cited from \cite{Polymath} and \cite{PlattTrudgian} have
been checked against the arXiv sources and are applied on domains
where their hypotheses hold, the one caveat being
Remark~\ref{rem:1044}. Items resolved during the review are also
removed from this list: the computer-assisted proof of
Proposition~\ref{prop:sweep} has been reproduced on an independent
toolchain and its program reviewed --- see the proof given in
Section~\ref{sec:stored} --- the proofs of
Lemmas~\ref{lem:frozen}, \ref{lem:contraction}, \ref{lem:tzero}
and~\ref{lem:prism} have been verified in detail in the course of
the review; the formerly outlined proofs of
Lemma~\ref{lem:reduction}, Lemma~\ref{lem:derivbox} and
Proposition~\ref{prop:transfer} have been completed in full,
through Lemmas~\ref{lem:capdecay}--\ref{lem:ydecay}
and~\ref{lem:errmaj}, Lemmas~\ref{lem:dompoint}
and~\ref{lem:headtail} with Proposition~\ref{prop:derivgates},
and Lemmas~\ref{lem:sigmaslope}--\ref{lem:cells} with
Proposition~\ref{prop:dinicells}, respectively. No analytic
items remain.)

\subsection*{The remaining gaps}
\begin{itemize}
\item[\textbf{\ref{gap:dinicells}.}] The computer-assisted proof
of Proposition~\ref{prop:dinicells} (the Dini cell certificates;
worst ratio within $1.4\times10^{-6}$ of failing, the
second-thinnest margin in the proof).
\item[\textbf{\ref{gap:finiteerr}.}] The computer-assisted proof
of Proposition~\ref{prop:finiteerr} (the uniform error bound
$E_{\max}$ over the finite region; a small computation, not yet
independently reproduced).
\item[\textbf{\ref{gap:tailcomp}.}] The computer-assisted proof of
Proposition~\ref{prop:tailcert} ($1-D<2.8\times10^{-4}$; post-error
margin $\approx1.7\times10^{-4}$).
\item[\textbf{\ref{gap:unifboxerr}.}] The computer-assisted proof
of Proposition~\ref{prop:barriererr} (window-index constancy,
$N^2<x/4\pi$, and the uniform error enclosure on the barrier box;
a small computation).
\item[\textbf{\ref{gap:barriercomp}.}] The computer-assisted
proofs of Propositions~\ref{prop:prismcert}
and~\ref{prop:tailexp} (the $883$-prism certificate, coefficient
regeneration, and tail-exponent gate; comfortable margins except
the tight Proposition~\ref{prop:tailexp}, but correctness rests on
the C sources and FLINT/Arb).
\end{itemize}
Each item should be verified by reading the certifying program
against the corresponding statement in this paper and rerunning it
on an independently built toolchain, following the protocol used
for Proposition~\ref{prop:sweep} in Section~\ref{sec:stored}.
...
candidate uses them; every analytic argument now carries a complete
verified proof; and the open questions are confined to the certified
computations listed above. Modulo those remaining points, the logical
...
condition is a strict directed interval inequality with fail-closed
parsing. The thinnest quantitative margins are the finite-range
post-error margin ($5.6\times10^{-7}$), the height-transfer ratio
($1-\text{ratio}\approx1.4\times10^{-6}$), and the tail contraction
($1-D\approx2.8\times10^{-4}$); with all analytic arguments now
verified, these thin margins are the places where the certified
computations themselves must be most trustworthy, and their
independent reproduction should receive the closest scrutiny.
