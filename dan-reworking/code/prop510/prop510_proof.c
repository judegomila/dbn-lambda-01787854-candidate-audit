 ...
 * Proposition 5.10 of the review manuscript
 * (dan-reworking/latex/exposition/gomila-proof-exposition.tex): the certified
 * cutoff inequalities of the tail argument, at the cutoff
 * N_* = 3840000, uniformly for
 *
 *   t in I_t = [129/800, 161250001/10^9],
 *   y in I_box = [1872719/10^7, 23409/125000]   (for D and M_max),
 *   y in I_ext = [1872719/10^7, 8231039/10^7]   (for the error majorants),
 *
 * with mollifier support S = {1,2,3,4,5,6,7,10,11,13,14}, exact
 * rational vector lambda (sum |lambda_d| = 9.918746), head lengths
 * M = 153814 and M_err = 3000.  It certifies:
 *   - the cap-validity conditions (eq:capvalid), as
 *     sigma_i > (t/2) log N_* on both y-hulls (SC1, SC2);
 *   - the cutoff conditions (eq:gates) for every cap left endpoint
 *     (a = floor(M/d) for d in S at sigma_1; a = M at sigma_2;
 *     a = M_err at sigma_1 and sigma_2) (GN-TR, GN-AB, GN-error);
 *   - the A-side slope condition (eq:ym) (YM);
 *   - the hypotheses of the monotonicity lemmas of Section 5
 *     (structural gates, SC3, and the Delta / e_C0 quotient gates);
 *   - D(N_*) < 0.999721, M_max < 1.608290, and
 *     E_AB + E_C < 1.1672e-8, with D assembled per (eq:D) as
 *     P + TR + OV + M_max * AB from an independently computed exact
 *     Dirichlet convolution;
 *   - the normalized margin (1-D)/M_max - (E_AB+E_C) > 0.00017352.
 * Every quantity is enclosed in Arb ball arithmetic (precision from
 * argv, default 320 bits, refused below 256); every decisive
 * comparison is an Arb strict comparison and therefore fails closed
 * when balls overlap.  The program reads no input files.
 *
 * PROVENANCE AND MODIFICATIONS
 * ----------------------------
 * Derived from verifiers/verify_tail_arb.c in the candidate
 * repository (dbn-lambda-01787854-candidate-audit), Gomila's own
 * from-scratch reimplementation of his earlier Python tail verifier
 * (no code, cached decimal, or serialized coefficient shared).
 * Changes made for this review:
 *   - this header replaces the original one;
 *   - the margin constant displayed in Proposition 5.10 is now
 *     itself certified: a strict ball comparison
 *     margin > 17352/10^8 was added alongside the original
 *     margin > 0 check (search for "sharp");
 *   - the repository-internal codename "P1113" in the banner and in
 *     two check labels is replaced by neutral wording, and the
 *     final verdict lines use the RESULT PASS / RESULT FAIL format
 *     of this review's other programs.
 * Apart from the listed changes, every statement is line-for-line
 * identical to the original; diff against
 * verifiers/verify_tail_arb.c to audit.
 * =========================================================================== */
...
main(int argc, char **argv)
{
    ...
    printf("prop510 tail verifier (Arb): precision=%ld, N1=%lu, M=%d\n",
           (long) wp, N1, MHEAD);
    ...
    report("mollifier has lambda_1 = 1",
    ...
    report("exact mollifier vector normalization sum|lambda|=9918746/1000000",
    ...
    /* sharp: the margin constant displayed in Proposition 5.10, as the
       exact rational 17352 / 10^8. */
    report("sharp margin (1-D)/Mmax - err > 0.00017352",
           gt_q(margin, 17352, 100000000UL));
    ...
    puts(failures == 0
         ? "RESULT PASS: certified cutoff inequalities (Proposition 5.10)"
         : "RESULT FAIL");
