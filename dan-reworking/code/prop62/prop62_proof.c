 ...
 *     t in [0, 129/800],
 *
 * (1) the window index N = floor(sqrt(x/(4*pi) + t/16)) equals 690988
 *     at the two extreme corners (X, t=0) and (X+1, t=129/800), hence
 *     everywhere on the box by monotonicity (see the proof block in
 *     the manuscript);
 * (2) N^2 < X/(4*pi), hence N^2 < x/(4*pi) on the whole box; and
 * (3) the error terms of Theorem 1.3 of [Polymath, arXiv:1904.12438v2],
 *     bounded via the displays (20)--(23) there and the conservative
 *     10.50 corollary of its Proposition 6.6(vi) (eq. (eq:ec0) of the
 *     manuscript), satisfy on the box, for 0 < t <= 129/800,
 *
 *         e_A + e_B + e_C0 < 0.000356523011600040 < 0.00125 = 1/800.
 *
 * All quantities are enclosed in 256-bit Arb ball arithmetic from the
 * exact integer/rational inputs above; every acceptance condition is
 * a strict ball inequality (arb_lt / arb_is_positive), which fails
 * closed. The t-range is covered by hand-verified monotonicity in t
 * of each majorant (each is evaluated at the worst endpoint), and the
 * y-range likewise; the reduction is spelled out in the manuscript's
 * proof block. The program reads no input files. The t=0 endpoint of
 * the error bound is excluded from (3) (Theorem 1.3 requires t > 0);
 * the extension to t=0 is analytic (Lemma 6.3 of the manuscript).
 *
 * PROVENANCE AND MODIFICATIONS
 * ----------------------------
 * Derived from barrier/src/verify_uniform_error_01787854.c in the
 * candidate repository (dbn-lambda-01787854-candidate-audit).
 * Changes made for this review:
 *   - this header replaces the original one;
 *   - the sharp constant displayed in Proposition 6.2 is now itself
 *     certified: a strict ball comparison of the error total against
 *     356523011600040 * 10^-18 was added alongside the original
 *     comparison against 0.00125 (search for "sharp");
 *   - the final verdict lines are reworded (RESULT PASS / RESULT
 *     FAIL, and the scope note now cites Lemma 6.3 of the
 *     manuscript);
 *   - the include of the Arb header is <flint/arb.h> instead of
 *     "arb.h" (FLINT >= 3 header layout, as in ../prop43).
 * Apart from the listed changes, every statement is line-for-line
 * identical to the original; diff against
 * barrier/src/verify_uniform_error_01787854.c to audit.
 * =========================================================================== */
...
main(void)
{
    ...
    arb_t ec_factor1, ec_exp2, ec_exp3, eC0, total, epsilon, sharp;
    ...
    arb_init(epsilon); arb_init(sharp);
    ...
    */
    ...
    print_ball("uniform conservative error total upper", total);
    print_ball("winding allowance", epsilon);

    /* sharp: the constant displayed in Proposition 6.2, as the exact
       rational 356523011600040 / 10^18. */
    arb_set_si(sharp, 356523011600040L);
    arb_div_ui(sharp, sharp, 1000000000000000000UL, prec);
    ok &= strict_lt("uniform error total < 0.000356523011600040 (sharp)",
                    total, sharp);
    ok &= strict_lt("uniform conservative error total < 0.00125",
                    total, epsilon);

    printf("%s\n", ok
        ? "RESULT PASS: uniform error bound on the barrier box "
          "(Proposition 6.2)"
        : "RESULT FAIL");
    printf("SCOPE NOTE: the error bound is for 0 < t <= 129/800; the "
           "extension to t=0 is analytic (Lemma 6.3 of the manuscript).\n");
    ...
    arb_clear(epsilon); arb_clear(sharp);
