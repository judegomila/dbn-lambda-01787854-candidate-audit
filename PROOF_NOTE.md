...
nontrivial zero in (2.1). The real-height endpoint contributes no zeta zero;
\(s=1\) is a pole, not a zero. This supplies hypothesis (i) without assuming
RH beyond the verified finite height.
...
The direct Arb runs at 180, 256, and 384 bits prove the required strict
inequalities; the worst stored ratio satisfies

\[
\text{ratio}<0.99999860767275095<1.
\tag{4.10}
\]
...
The target error verifier uses \(x-6.66\) in equation (23) and the paper's
\(x-12\) in equation (24). It obtains
...
\(e_A+e_B\) and the displayed equation (24), with \(x-12\), for
\(e_{C,0}\). Hence
...
Write \(g_t=H_t/B_t\). A separate 256-bit Arb calculation evaluates the
displayed Theorem 1.3 bounds over the complete
...
\(H_t\), \(B_t^{\pm1}\), the fixed finite sum \(f_t\), and the displayed
...
The sealed result consists of 883 consecutive closed prisms from exactly
\(0\) through \(129/800\), with byte-identical adjacent seams. Every prism
has winding zero, and the independently recomputed minimum margin exceeds
