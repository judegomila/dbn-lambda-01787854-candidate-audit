/*
 * Independent FLINT/Arb verification of the lower-time P1113 tail interface
 * for the 0.1782354 candidate row:
 *
 *   t in [160700000/10^9, 160700001/10^9],
 *   y0^2 = 87677/2500000,
 *   1 - 2*t0 = 3393/5000,
 *   N >= N1 = 4050000,
 *   M = 153814.
...
exact_hull_checks(void)
{
     ...
     * below UINT64_MAX (the largest is < 1.1e19).
     */
    ...
    uint64_t ytop2_scaled =
        UINT64_C(8237719) * UINT64_C(8237719) * UINT64_C(5000);
    uint64_t top_target_scaled =
        UINT64_C(3393) * UINT64_C(10000000) * UINT64_C(10000000);
    uint64_t yprev2_scaled =
        UINT64_C(8237718) * UINT64_C(8237718) * UINT64_C(5000);
    ...
    uint64_t yhi_tupper_right =
        (UINT64_C(1000000000) - 2 * UINT64_C(160700001))
...
main(int argc, char **argv)
{
    ...
    printf("P1113 lower-time Arb verifier: precision=%ld, N1=%lu, M=%d\n",
           (long) wp, N1, MHEAD);
    ...
    set_q(tlo, 160700000, 1000000000UL);
    set_q(thi, 160700001, 1000000000UL);
    arb_union(t, tlo, thi, wp);
    set_range_q(ybox, 1872719, 10000000UL, 23409, 125000UL);
    set_range_q(yext, 1872719, 10000000UL, 8237719, 10000000UL);

    report("exact rational target and y-hull containments", exact_hull_checks());
    report("exact target identity t0+y0^2/2 = 891177/5000000",
           1607UL * 500UL + 87677UL == 891177UL);
    report("exact final-height identity 1-2*t0 = 3393/5000",
           10000UL - 2UL * 1607UL == 2UL * 3393UL);
    ...
    report("Delta N-monotonicity gate: (t^2/16)L(L-2)+0.626 > 0",
           arb_gt(tmp, zero));
    report("eC0 quotient N-monotonicity gate: 3 < 10.44 and X_N>12",
           3 * 100 < 1044 && gt_q(x1, 12, 1));
     ...
     */
    report("P corridor 0.169319 < P < 0.169322",
           gt_q(P, 169319, 1000000) && lt_q(P, 169322, 1000000));
    report("TR corridor 0.263610 < TR < 0.263614",
           gt_q(TR, 263610, 1000000) && lt_q(TR, 263614, 1000000));
    report("OV corridor 0.038778 < OV < 0.038781",
           gt_q(OV, 38778, 1000000) && lt_q(OV, 38781, 1000000));
    report("Mmax corridor 1.608212 < Mmax < 1.608216",
           gt_q(Mmax, 1608212, 1000000) && lt_q(Mmax, 1608216, 1000000));
    report("AB corridor 0.326169 < AB < 0.326172",
           gt_q(AB, 326169, 1000000) && lt_q(AB, 326172, 1000000));
    report("D corridor 0.996262 < D < 0.996265",
           gt_q(D, 996262, 1000000) && lt_q(D, 996265, 1000000));
    report("P interval width is smaller than 1-D", arb_lt(P_width, one_minus_D));
    report("decisive contraction D < 1", arb_lt(D, one));
    report("flow corridor 0.0023230 < flow < 0.0023236",
           gt_q(flow, 23230, 10000000) && lt_q(flow, 23236, 10000000));
    report("error corridor 1.0936e-8 < err < 1.0939e-8",
           gt_q(err, 10936, 1000000000000UL)
           && lt_q(err, 10939, 1000000000000UL));
    ...
    puts(failures == 0 ? "RESULT: ALL ARB LOWER-TIME TAIL CHECKS PASS"
                      : "RESULT: ARB LOWER-TIME TAIL CHECK FAILURE");
