#!/usr/bin/env python3
"""Verify the complete finite Triangle sweep and its normalized binding.
...
P11_TBOX = ("16125/100000", "16125/100000")
LATER_TBOX = (
    "161250000/1000000000",
    "161250001/1000000000",
)

FILES = (
    ("p235711_690988_690995.log.gz", "p235711", 690_988, 690_995),
    ("p235711_690996_691500.log.gz", "p235711", 690_996, 691_500),
    ("p235711_691501_697000.log.gz", "p235711", 691_501, 697_000),
    ("p235711_697001_728999.log.gz", "p235711", 697_001, 728_999),
    ("p2357_729000_818999.log.gz", "p2357", 729_000, 818_999),
    ("p235_819000_1027999.log.gz", "p235", 819_000, 1_027_999),
    ("p23_1028000_1030000.log.gz", "p23", 1_028_000, 1_030_000),
    ("p23_1030001_1050000.log.gz", "p23", 1_030_001, 1_050_000),
    ("p23_1050001_1100000.log.gz", "p23", 1_050_001, 1_100_000),
    ("p23_1100001_1300000.log.gz", "p23", 1_100_001, 1_300_000),
    ("p23_1300001_1700000.log.gz", "p23", 1_300_001, 1_700_000),
    ("p23_1700001_2200000.log.gz", "p23", 1_700_001, 2_200_000),
    ("p23_2200001_2800000.log.gz", "p23", 2_200_001, 2_800_000),
    ("p23_2800001_3300000.log.gz", "p23", 2_800_001, 3_300_000),
    ("p23_3300001_3840000.log.gz", "p23", 3_300_001, 3_840_000),
)
...
def parse_file(path, family, expected_lo, expected_hi):
    expected_tbox = P11_TBOX if family == "p235711" else LATER_TBOX
    require_weight = family != "p235711"
    first = None
    last = None
    previous = None
    rows = 0
...
def main():
    ...
    if total_rows != expected_total or total_rows != 3_149_013:
    ...
    for family, expected in EXPECTED_LEGS.items():
        ...
        if actual != expected:
            raise AssertionError(
                f"{family}: got {actual}, expected {expected}"
            )
        print(
    ...
    if global_t_floor != F(791366, 10**12):
        raise AssertionError(f"unexpected global floor {global_t_floor}")
    if not (0 < error_max < F(234, 10**9)):
        raise AssertionError(f"unexpected Emax {error_max}")
    if not (binding_floor > F(557, 10**9)):
    ...
    print(
        ...
        "B=893927/5000000 rows=3149013 "
        "N=690988..3840000 gaps=0 overlaps=0 UNCERT=0 "
