#!/usr/bin/env python3
"""Prove each candidate tail is the deposited engine plus declared patches."""
...
HEADER = (
    "# AUDIT PATCH TRI178785400SAFE: pristine deposited engine plus the delimited\n"
    "# parameter block below and the conservative 10.44 -> 10.50 error weld.\n"
)
BEGIN = (
    "\n# =====================================================================\n"
...
def restore_deposited_source(path: Path, precision: int) -> str:
    ...
    conservative_error = "itv(F(1050, 100))"
    deposited_error = "itv(F(1044, 100))"
    if text.count(conservative_error) != 1:
        raise AssertionError(f"{path.name}: unexpected conservative error patch")
    text = text.replace(conservative_error, deposited_error, 1)
...
for target, precision in TARGETS:
    ...
    print(
        ...
        f"precision={precision}, MHEAD_MAX=153814, 10.50 error weld, "
        "TRI178785400SAFE block"
    )
