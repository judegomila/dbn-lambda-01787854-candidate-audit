#!/usr/bin/env python3
"""Exact/interval audit of the finite x-window freeze at the 0.1782354 row.
...
def source_binding(repo: Path) -> None:
    source = repo / "src" / "lemma_sweep_p23571113.c"
    driver = repo / "scripts" / "run_finite_01782354.sh"
    theorem = repo / "WINDOW_FREEZE_THEOREM.md"
    tail = repo / "verifiers" / "verify_tail_01782354_arb.c"
    ...
    check(
        "S10 exact lower-time t,y row is present",
        "16070 16070 100000 350708 10000000" in drv,
    )
    check(
        "S11 all five prime modes and exact endpoints are present",
        all(token in drv for token in (
            "690988 728999 7", "729000 774999 6",
            "775000 849999 5", "850000 1074999 4",
            "1075000 1100000 3", "3300001 4050000 3",
        )),
    )
    check(
        "S12 lower-time Arb tail verifier exists",
        tail.is_file(),
    )
    if tail.is_file():
        ...
        check(
            ...
            "#define N1 4050000UL" in tail_text
            and "891177/5000000" in tail_text,
            "finite and tail overlap at N=4050000",
        )
...
def main() -> None:
    ...
    check("A1 exact y ceiling", YMAX_SQ == F(3393, 5000))
    ...
    print(
        "CERTIFIED WINDOW-FREEZE CONCLUSION: at t0=1607/10000, for every "
        ...
        "bound, uniformly for y in [sqrt(87677/2500000),sqrt(3393/5000)]. "
