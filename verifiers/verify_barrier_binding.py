...
# For the conservative Proposition 6.6(vi) corollary, drop the favorable
# negative heat term.  The first factor is largest at (X,y_min), the cosh
# factor at y=1, and the last factor is bounded using its largest numerator
# and smallest denominator.
e_c0 = iv.exp(
...
check(
    "C4 conservative e_C0 bound is below 3.57e-4",
    upper(e_c0) < mp.mpf("3.57e-4"),
)
check(
...
uniform_matches = re.findall(
    r"^uniform conservative error total upper = (.+)$",
    uniform_log,
    re.MULTILINE,
)
uniform_ball = (
...
check(
    "F8 Arb independently certifies the conservative error below 0.00125",
