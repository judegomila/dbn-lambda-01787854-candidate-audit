...
the active set is exactly \(\{d_j,\ldots,d_s\}\).  Hence all nonempty active
sets requiring a gate are exhausted by
...
When \(n/N>d_s\), the active set is empty, so \(A=B=A'=0\); this common
zero-contribution case needs no inequality gate and is intentionally skipped
by the C loop.
